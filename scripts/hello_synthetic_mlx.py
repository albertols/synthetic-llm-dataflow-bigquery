...
def parse_args(argv: list[str]) -> argparse.Namespace:
  p = argparse.ArgumentParser(
      description="M4 MLX smoke test for synthetic-dataflow-bigquery")
  p.add_argument(
      "--ddl_path",
      required=True,
      help="Local path to _ddl.json (e.g. output/SRC_dataset/ddl_metadata_SRC_dataset_B_TABLE.json)"
  )
  p.add_argument(
      "--reference_table",
      required=True,
      help="BQ table FQN for reference SELECT, e.g. src_dataset.synthetic_data")
  ...
  p.add_argument(
      "--model_path",
      default="./models/gemma4/e4b-it/v1/",
...
def main(argv: list[str] | None = None) -> int:
  ...
  schema = TableSchema.model_validate(
      json.loads(Path(args.ddl_path).read_text(encoding="utf-8")))
  # A dynamically derived class: PascalCase is the right name for a class object.
  Record = derive_record_model(schema)  # noqa: N806  # pylint: disable=invalid-name
  record_schema = Record.model_json_schema()
  logger.info("Schema parsed: %s (%d columns)", schema.fqn, len(schema.columns))

  logger.info("Pulling %d reference rows from %s", args.reference_limit,
  ...
  if not ref_rows:
    logger.error(
        "No reference rows returned — cannot prompt MLX without anchors.")
    return 1
  ...
  logger.info(
      ...
      total_secs,
      total_secs / attempted if attempted else 0.0,
      len(schema.columns),
  )

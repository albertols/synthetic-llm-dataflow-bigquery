...
def fig_run_timeline():
  ...
  ax.bar([x + 1 for x in xs], [GEN_BUCKETS[x] for x in xs],
         width=1.8,
         color=AQUA,
         zorder=3,
         label="batches completed (1,000 rows each)")
  ...
  _title(
      ax, f"Only {gen_min} of {WALL_MIN:.0f} minutes actually generated rows",
...
def fig_where_time_went():
  ...
  _title(
      ax1, f"Where the {WALL_MIN:.0f} minutes went",
  ...
  _title(
      ax2, "Generation is not the bottleneck",
...
def fig_dedup_options():
  ...
  _style(ax1)
  _title(
      ax1, "Cost of each uniqueness strategy",
      "today's DAG chains up to three GroupByKeys — row digest, PK, identity")

  # Guarantee matrix.
  rows = [
      "Identity columns unique", "Full-row dups removed",
  ...
  for i in range(matrix.shape[0]):
    for j in range(matrix.shape[1]):
      ok = bool(matrix[i, j])
      ax2.add_patch(
          plt.Rectangle((j - 0.46, i - 0.42),
                        0.92,
                        0.84,
                        color=AQUA if ok else GRID,
                        alpha=0.85 if ok else 1.0,
                        zorder=2))
      ax2.text(
          j,
          i,
  ...
  _title(
      ax2, "What each one still guarantees",

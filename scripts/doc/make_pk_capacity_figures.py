...
def panel_curve(ax):
  ...
  shares = [
      expected_duplicate_share(1_000_000, int(1_000_000 * r)) for r in ratios
  ]
  ...
  ax.scatter([1.0], [one],
             ...
             linewidth=1.5)
  ax.annotate(
      f"capacity = rows\nstill {one:.1%} duplicates", (1.0, one),
      xytext=(0.12, 0.30),
      color=INK,
      fontsize=9,
      arrowprops={
  ...
  ax.scatter([FK_KEY_SAMPLE_MARGIN], [margin],
             s=55,
             color=AQUA,
             zorder=5,
             edgecolor=SURFACE,
             linewidth=1.5)
  ax.annotate(
      f"sizing margin x{FK_KEY_SAMPLE_MARGIN}\n{margin:.1%} duplicates",
      (FK_KEY_SAMPLE_MARGIN, margin),
      xytext=(14, 0.30),
      color=INK,
      fontsize=9,
      arrowprops={
  ...
  _style(ax)
  _title(
      ax, "Random PK draws are balls into bins",
...
def panel_ladder(ax):
  ...
  _style(ax)
  _title(
      ax, "C_TABLE: parent keys the child sees x 12",
...
def fig_pk_capacity():
  ...
  fig.savefig(
      ASSETS / "pk-capacity-random-draws.png", dpi=160, facecolor=SURFACE)
  plt.close(fig)
...
def main() -> None:
  ...
  print(f"{RUN}: {PK_DUPLICATES / NUM_ROWS:.1%} measured pk.duplicate; "
  ...
  for keys in (FK_KEY_SAMPLE_FLOOR, FK_KEY_SAMPLE_CEILING, PARENT_ROWS):
    ...
    print(f"  {keys:>10,} keys -> {cap:>12,} tuples: "

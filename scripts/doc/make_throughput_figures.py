...
GPU_BUSY_COLD = (
    (16.0, 17.5), (18.6, 28.4))  # embeds; vllm_spawn → last freetext_pool_built
GPU_BILLED_MIN_COLD = 16_420 / 60  # TotalGpuTime 16,420 GPU-s
AUTOSCALE_BOOT_COLD = 27.6  # 3 harness boots 15:01:10-18 (2 → 4 workers)
IDENTIFIER_FETCH_COLD = (
    28.4, 33.9)  # A_COL_005 944,582 values, REST paging (15:01:59 → 15:07:29)
...
RAMP_C_COLD = (1667, 2250, 2833, 2833, 11500, 10583, 10417, 10917, 9583, 10500,
               7917, 2333)
RAMP_C_WARM = (1500, 2417, 2833, 2000, 12167, 7833, 10500, 11417, 8750, 12333,
               7417, 4167)
...
BATCH_SECONDS_STEADY = (26, 29
                       )  # 10k-row batch_done seconds at full concurrency
...
SETUP_C_COLD = (18.2, 59.9, 60.6, 61.9, 62.1, 62.5, 64.9, 65.5, 66.2, 68.2,
                70.4, 71.1, 72.0, 72.0, 72.0, 74.7, 75.1, 75.3, 75.8, 76.0,
                76.1, 76.6, 77.2, 77.7, 89.9, 93.4, 99.2, 110.2, 127.2, 167.7,
                176.4, 444.4)
SETUP_C_WARM = (50.6, 61.5, 61.6, 61.7, 64.3, 64.7, 67.0, 67.2, 75.6, 75.7,
                75.8, 76.0, 76.2, 76.2, 76.6, 76.7, 76.8, 77.1, 77.5, 77.5,
                77.6, 77.7, 78.3, 78.8, 80.8, 82.1, 92.6, 101.6, 130.6, 190.8,
                203.3, 213.3)
...
POOLS_A_COLD = (37.9, 40.6, 40.7, 41.2, 41.4, 42.8, 43.0, 43.7, 44.1, 44.6,
                46.3, 46.4, 46.4, 47.1, 47.4, 48.7, 49.0, 49.4, 49.6, 51.4,
                51.7, 52.1, 55.2, 56.6, 57.4, 59.1, 86.2, 91.0, 101.4, 102.0,
                169.1, 180.9)
POOLS_A_WARM = (37.6, 38.4, 39.4, 40.7, 40.9, 41.4, 42.2, 42.7, 43.4, 44.2,
                44.6, 45.1, 45.5, 47.4, 47.6, 47.6, 48.2, 48.3, 48.9, 49.2,
                49.6, 50.0, 50.5, 52.2, 52.6, 53.2, 53.3, 53.7, 57.4, 58.2,
                59.0, 60.2)
...
ACCEPT_BATCH_SECONDS = (27.5, 27.5, 6.1, 6.6, None
                       )  # 10k-row batch_done at steady state
...
VLLM_READY_S = {
    ...
    "multi": 344.0,
    "multi warm": 183.0,
    "multi cold cpu": 598.7
}
MODEL_PULL_S = {"multi warm": 52.5, "multi cold cpu": 177.6}
POPULATION_STAGE_MIN = {
    "multi": 2.9,
    "multi cold cpu": 15.4
}  # RagEmbedChunks span (GPU x8 vs CPU x56)
...
FETCH_FAILURES = (
    107, 459
)  # source_values_arrow_fallback (PermissionDenied) → all fetches inactive
...
def _timeline(ax, phases, wall, *, autoscale_at, gpu_busy=(), extra=()):
  ...
  for i, (label, s, e, cls) in enumerate(phases):
    ...
    ax.barh(
        ...
        zorder=3,
        hatch=_CLASS_HATCH[cls],
        edgecolor=SURFACE if _CLASS_HATCH[cls] else "none")
  ...
  ax.axvline(
      autoscale_at, color=INK, linewidth=1.1, linestyle=(0, (3, 3)), zorder=2)
  ax.text(
      autoscale_at + 0.5,
      n - 0.2,
      "2 → 4 workers\n(harness boots)",
...
def fig_where_time_went():
  ...
  _timeline(
      ...
      PHASES_COLD,
      WALL_MIN_COLD,
      autoscale_at=AUTOSCALE_BOOT_COLD,
  ...
  ax2.legend(
      handles, [
          "Beam / launch / IO", "GPU (embed + vLLM pool ladders)",
  ...
  fig.tight_layout()
  fig.savefig(
      ASSETS / "throughput-where-time-went.png", dpi=160, facecolor=SURFACE)
  plt.close(fig)
...
def fig_generation_ramp():
  ...
  fig, (ax1, ax2) = plt.subplots(
      1, 2, figsize=(13.5, 5.2), facecolor=SURFACE, sharey=True)
  for ax, series, regs, title in (
      (ax1, (("cold", RAMP_C_COLD, BLUE), ("warm", RAMP_C_WARM, AQUA)),
       (SDK_REGISTER_C_COLD,
        SDK_REGISTER_C_WARM), "C_TABLE — first generate stage of the job"),
    ...
    for i, reg in enumerate(regs):
      ...
      for r in reg:
        ax.axvline(
            r, color=colour, linewidth=0.9, linestyle=(0, (2, 3)), zorder=2)
    ax.axhline(
        FLEET_ROWS_PER_S_MEASURED,
        color=MUTED,
        linewidth=1.0,
        linestyle=(0, (4, 3)),
        zorder=2)
    ax.set_xlabel(
        "minutes since the stage's first batch_done", color=MUTED, fontsize=9)
    _style(ax)
  ...
  ax1.text(
      23.5,
      FLEET_ROWS_PER_S_MEASURED * 1.04,
      f"steady {FLEET_ROWS_PER_S_MEASURED / 1e3:.1f}k rows/s",
      ha="right",
      color=MUTED,
      fontsize=8.5)
  ax1.annotate(
      "autoscaled workers'\nSDK harnesses register",
      xy=(SDK_REGISTER_C_COLD[1], 4_200),
      xytext=(12.6, 4_600),
      color=INK,
      fontsize=8.2,
      linespacing=1.25,
  ...
  ax1.set_ylabel(
      "rows / s (2-min buckets, fleet-wide)", color=MUTED, fontsize=9)
  ax2.text(
      1.2,
      17_300,
      ...
      linespacing=1.3)
  fig.suptitle(
      "The first stage ramps for 8 minutes at a quarter of the fleet's steady state",
      color=INK,
      fontsize=12.5,
      fontweight="600",
      x=0.01,
  ...
  fig.tight_layout()
  fig.savefig(
      ASSETS / "throughput-generation-ramp.png", dpi=160, facecolor=SURFACE)
  plt.close(fig)
...
def fig_setup_cost():
  ...
  for i, (_, values, colour) in enumerate(groups):
    ...
    ax.plot([i - 0.28, i + 0.28], [med, med],
            ...
            zorder=4)
    ax.text(
        i + 0.32, med, f"p50 {med:.0f} s", color=INK, fontsize=8.5, va="center")
    ax.text(
        i,
        1.9,
        f"n={vals.size}\nΣ {vals.sum():,.0f} s",
...
def fig_shuffle_barriers():
  ...
  for i, (_, gb, projected) in enumerate(bars):
    ax.bar(
        ...
        gb,
        width=0.5,
        color=BLUE if not projected else AQUA,
        zorder=3,
        hatch="//" if projected else None,
        edgecolor=SURFACE if projected else "none")
    ax.text(
        i,
        gb + 3,
        f"{gb:.0f} GB",
...
def fig_gil_ceiling():
  ...
  bars = (
      ("1 interpreter / worker\nmeasured, R6 pair", FLEET_ROWS_PER_S_MEASURED,
       False),
      ("8 interpreters / worker\nmeasured, R7 multi (4 workers)",
       MULTI_PEAK_ROWS_PER_S, False),
      ("8 interpreters / worker\nlinear projection",
  ...
  for i, (_, f, projected) in enumerate(bars):
    ax.bar(
        ...
        width=0.55,
        color=AQUA,
        alpha=0.55 if projected else 1.0,
        hatch="//" if projected else None,
        edgecolor=SURFACE if projected else "none",
        zorder=3)
    ax.text(
  ...
  ax.annotate(
      f"x{gain:.1f} measured",
      xy=(1, MULTI_PEAK_ROWS_PER_S),
      xytext=(0.45, 52_000),
      color=INK,
      fontsize=9.5,
      fontweight="600",
  ...
  ax.text(
      0.45,
      62_000, "10k-row batches: 26-29 s on one GIL → 5.6-6.7 s with eight;\n"
...
def fig_evolution():
  ...
  for label, mins in ACCEPT_PHASES_MIN.items():
    ...
    ax.barh(
        ...
        edgecolor=SURFACE if label in hatches else "none",
        zorder=3,
        label=label)
    for i, (lo, v) in enumerate(zip(left, vals, strict=True)):
      if v >= _MIN_LABELLED_PHASE_MIN:
        ax.text(
            ...
            ha="center",
            va="center",
            color=SURFACE if colours[label] != AQUA else INK,
            fontsize=8.6)
    left = left + vals
  for i, (wall, note) in enumerate(
      zip(ACCEPT_WALL_MIN, ACCEPT_WORKERS_AT_4_MIN, strict=True)):
    ax.text(
        wall + 1.0,
        i,
        f"{wall:.1f} min · {note}",
  ...
  ax.set_xlim(0, 140)
  ax.set_xlabel(
      "minutes (phases on the critical path; cleanup omitted)",
      color=MUTED,
      fontsize=9)
  ax.legend(
      frameon=False, fontsize=8.8, labelcolor=INK, loc="lower right", ncol=2)
  _style(ax, grid_axis="x")
  _title(
...
def check_palette():
  ...
  ks = list(names)
  print(
      f"palette separation (OKLab dE x100, normal vision; floor = {CVD_FLOOR:.0f}):"
  )

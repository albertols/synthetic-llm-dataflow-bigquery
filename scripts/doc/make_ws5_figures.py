...
def _title(ax, text, sub=None):
  ax.set_title(
      text, color=INK, fontsize=12, fontweight="600", loc="left", pad=26)
  if sub:
    ax.text(
        0,
        1.025,
...
def fig_cost_anatomy():
  ...
  vals = [
      POOL_BUILD_LLM, POOL_BUILD_WALL, VLLM_SPAWN, MODEL_PULL, CHUNK_REUSE,
      INDEX_BUILD
  ]
...
def fig_autoscale_amplification():
  ...
  ax.annotate(
      ...
      xy=(LAST_POOL_MIN, 13.5),
      xytext=(LAST_POOL_MIN - 17, 17.5),
      color=ORANGE,
      fontsize=9.5,
      fontweight="600",
      ha="left",
  ...
  fig.tight_layout()
  fig.savefig(
      ASSETS / "ws5-autoscale-amplification.png", dpi=160, facecolor=SURFACE)
  plt.close(fig)
...
def _modes(rng):
  """Synthetic 2-D stand-in for a column's embedding manifold: 5 modes of
    ...
    are the real ones."""
  centers = np.array([[0.0, 0.0], [2.6, 1.1], [-2.3, 1.5], [1.4, -2.4],
                      [-1.8, -2.2]])
...
def _kcenter(pts, k, start=None):
  first = int(np.argmin(np.linalg.norm(pts - pts.mean(axis=0),
                                       axis=1))) if start is None else start
  chosen = [first]
...
def fig_seed_strategies():
  ...
  rot = [
      _kcenter(pts, k),
      _kcenter(pts, k, start=int(np.argmax(pts[:, 0]))),
  ...
  titles = [
      ...
      ("--pool_seed_strategy=kcenter_rotate",
       "arm A — seeds rotate per ladder attempt"),
  ]

  for ax, (name, sub) in zip(axes, titles, strict=True):
    ax.scatter(
        pts[:, 0], pts[:, 1], s=13, color=GRID, edgecolors="none", zorder=1)
    key = name.split("=")[1]
    if key == "kcenter_rotate":
      marks = [("o", ORANGE, "attempt 1"), ("s", BLUE, "attempt 2"),
               ("^", AQUA, "attempt 3")]
    ...
    ax.text(
        0.5,
        -0.11,
        f"modes reached by the 8 seeds: {covered} of 5{note}",
        transform=ax.transAxes,
        ha="center",
        color=INK,
        fontsize=9.5,
...
def fig_sampler_hoisting():
  ...
  ax1.set_ylabel(
      "CPU-seconds across the 1M-row job (log)", color=MUTED, fontsize=9)
  ...
  _title(
      ax1, "Re-materializing observed values dominates Generate",
  ...
  ax2.bar(
      ...
      color=BLUE,
      zorder=3,
      label="hoisted to sampler construction")
...
def check_palette():
  ...
  for i in range(len(ks)):
    for j in range(i + 1, len(ks)):
      ...
      print(
          f"  {ks[i]:6s} vs {ks[j]:6s}: {de:5.1f}  {'PASS' if de >= CVD_FLOOR else 'FAIL'}"
      )

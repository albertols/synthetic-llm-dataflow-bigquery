...
# Positional concept: C2E3-prefixed 24-char upper-hex ids (COL_001-class)
...
def fig_evidence():
  ...
  for ax, ks, run, table in (
    ...
    _title(
        ax, f"{table} R1 — numeric decile-KS",
  ...
  for ax, gaps, table in (
    ...
    _style(ax, grid_axis="x")
    _title(
        ax, f"{table} R1 — categorical entropy gap",
        "top 8 · negative = synthetic MORE uniform than source · "
        "no rule fired")
    ax.set_xlim(-1.0, 0.05)
    ax.set_xlabel(
        "entropy gap (source - synthetic, normalized)", color=MUTED, fontsize=9)

  fig.tight_layout(pad=1.6)
  fig.savefig(
      ASSETS / "marginal-wave3-evidence.png", dpi=160, facecolor=SURFACE)
  plt.close(fig)
...
def fig_structure_loss():
  ...
  _style(ax)
  _title(
      ax, "shape-share inversion (crosscheck, % of sampled rows)",
  ...
  _style(ax)
  _title(
      ax, "identifier structure loss (crosscheck)",
  ...
  fig.tight_layout(pad=1.6)
  fig.savefig(
      ASSETS / "marginal-wave3-structure-loss.png", dpi=160, facecolor=SURFACE)
  plt.close(fig)
...
def fig_invcdf_band():
  ...
  fig.tight_layout(pad=1.6)
  fig.savefig(
      ASSETS / "marginal-wave3-invcdf-band.png", dpi=160, facecolor=SURFACE)
  plt.close(fig)
...
def fig_positional():
  ...
  for ax, values, name, note in (
      (axes[0], hex_ids, "C2E3-prefixed 24-char upper-hex (COL_001-class)",
    ...
    colors = [
        AQUA if s == 1 else BLUE if s <= NARROW_MAX else MUTED for s in sizes
    ]
  ...
  fig.tight_layout(pad=1.6)
  fig.savefig(
      ASSETS / "marginal-wave3-positional.png", dpi=160, facecolor=SURFACE)
  plt.close(fig)

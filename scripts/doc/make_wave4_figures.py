...
#   COL_015 REEMB.T : 30,936/96,815    vs 15,499/45,903     vs alpha ~0.003
PANEL_VS_TRUTH = (
    ("COL_054 (A)\n'BATCH'", 0.5395, 0.5348, 0.7508),
    ("COL_053 (A)\n'ZZ3000'", 0.5342, 0.5300, 0.7460),
    ("COL_024 (B)\n'35'", 0.9184, 0.9201, 0.3092),
    ("COL_015 (B)\n'REEMBOLSOS T'", 0.3196, 0.3376, 0.0030),
)
...
def fig_panel_vs_truth():
  ...
  for xi, (a, b, c) in enumerate(zip(src, syn, panel, strict=True)):
    for dx, v in ((-w, a), (0, b), (w, c)):
      ax.text(xi + dx, v + 1.5, f"{v:.1f}", ha="center", color=INK, fontsize=8)
  ax.set_xticks(x, labels, fontsize=9, color=MUTED)
  ax.set_ylabel(
      "dominant-value share of substantive rows (%)", color=MUTED, fontsize=9)
  ax.set_ylim(0, 100)
  _style(ax)
  _title(
      ax, "The engine matched the exact marginal; the panel did not",
...
# --------------------------------------------------------------------------
def _old_capped_draw(values: list[str], rng: random.Random,
                     n: int) -> list[str]:
  ...
  for _ in range(n):
    ...
    for m, c in kept:
      ...
      r -= c
    out.append(
        sample_from_mask(mask, alphabets, rng.randrange, positional=positional))
  return out
...
def fig_mask_entropy():
  ...
  ax = axes[0]
  names = [
      "true v4 process", "capped table\n(pre-wave-4)", "tail bucket\n(wave 4)"
  ]
  distinct = [
      len(set(src_masks)),
  ...
  for xi, s in enumerate(shares):
    ax.text(
        xi, s * 100 + 0.8, f"{s * 100:.1f}", ha="center", color=INK, fontsize=9)
  ax.set_xticks(x, names, fontsize=9, color=MUTED)
  ax.set_ylabel("digit share of mask positions (%)", color=MUTED, fontsize=9)
  ax.set_ylim(0, 100)
  _style(ax)
  _title(
      ax, "Digit skew of the lexicographic tie-break",
...
def fig_domain_weights():
  ...
  split = build_identifier_artifacts(
      tuple("x" * 10), None, rows, domain=frozenset(domain))
  ...
  _style(ax)
  _title(
      ax, "A distinct-valued domain must not out-vote the sample",
...
def _band_concept():
  ...
  draws = np.rint(
      np.interp(rng.random(N_NUM_DRAWS), np.linspace(0, 1, obs.size),
                obs)).astype(int)
...
def fig_numeric_scrub():
  ...
  _title(
      ax, "A_TABLE INT64 substantive copy",
  ...
  _style(ax)
  _title(
      ax, "Dense-band collisions and the scrub",

...
def panel_fanout(ax):
  ...
  ax.text(
      ...
      f"= {children:,} child rows",
      color=INK,
      fontsize=9)
  ax.set_xticks(ks)
  ax.set_xlabel(
      "children per parent key (k, source histogram)",
      color=MUTED,
      fontsize=9.5)
  ax.set_ylabel("parent keys", color=MUTED, fontsize=9.5)
  _style(ax)
  _title(
      ax, "The child's size is the parent's fan-out",
...
def panel_cells(ax):
  ...
  ax.plot(
      ...
      marker="o",
      markersize=5,
      label="random draw: 1 - C/k (1 - e^(-k/C))")
  ax.plot(
      ks, [0.0] * len(ks),
      color=AQUA,
      linewidth=2.4,
      marker="o",
      markersize=5,
      label="without replacement (k <= C): 0")
  ax.axvline(CELLS, color=BLUE, linewidth=1.2, linestyle="--")
  ax.text(
      CELLS - 0.15,
      0.5, f"C = {CELLS} cells\n(sample's joint\n"
      f"(C_COL_002, D_COL_018))",
      ha="right",
      color=BLUE,
      fontsize=9)
  ax.annotate(
  ...
  ax.set_ylabel(
      "expected pk.duplicate share inside the key", color=MUTED, fontsize=9.5)
  ax.legend(loc="upper left", frameon=False, fontsize=9, labelcolor=INK)
  _style(ax)
  _title(
      ax, "Inside a key: draw the PK cells without replacement",

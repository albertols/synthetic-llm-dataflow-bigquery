...
def _title(ax, text, sub=None):
  pad = 38 if sub and "\n" in sub else 26
  ax.set_title(
      text, color=INK, fontsize=12.5, fontweight="600", loc="left", pad=pad)
  if sub:
    ax.text(
        0,
        1.03,
...
def fig_orphan_rate():
  ...
  ax = axes[0]
  bars = (
      ("declared\ninformational\n(what ran)", _measured_orphan_rate(), ORANGE,
       False),
  ...
  for xi, (_, rate, color, hatched) in zip(x, bars, strict=True):
    ax.bar(
        ...
        edgecolor=SURFACE if hatched else color,
        linewidth=0,
    )
  ...
  ax.set_ylim(0, 1.14)
  ax.set_ylabel("share of child rows with no parent", color=MUTED, fontsize=9)
  ax.text(
      1,
      -0.28,
      "hatched = derived bound, not measured: with per-column "
  ...
  _style(ax)
  _title(
      ax, "Per-column enforcement is worse than none",
  ...
  height = 0.34
  ax.barh(
      y + height / 2, [c[1] for c in FK_COLUMNS],
      height=height,
      color=BLUE,
      label="parent landed distinct")
  ax.barh(
      y - height / 2, [c[2] for c in FK_COLUMNS],
      height=height,
      color=MUTED,
      label="child landed distinct")
  ...
  _title(
      ax, "The child invented values the parent lacks",
...
def fig_feasible_set():
  ...
  ax.imshow(
      grid,
      cmap=matplotlib.colors.ListedColormap([GRID, BLUE]),
      origin="lower",
      aspect="auto",
  ...
  _title(
      ax, "Parent keys are a sparse subset of the grid",
  ...
  for support, color, dash in (
    ...
    line, = ax.plot(
        ...
        linewidth=2.2,
        markersize=6,
        label=f"{support} values per column")
    if dash:
  ...
  ax.set_ylabel(
      "P(an independent draw hits a real key)", color=MUTED, fontsize=9)
  ax.legend(frameon=False, fontsize=8.5, labelcolor=INK, loc="lower left")
  _style(ax)
  _title(
      ax, "Independence survives only below the key count",
...
def fig_marginal_fit():
  ...
  child_rows = [{
      "A": rng.choices(a_values, weights=weights)[0],
      "B": rng.randrange(20)
  } for _ in range(4000)]
  keys = set()
  ...
  ax.bar(
      ...
      width,
      color=BLUE,
      label="child's own marginal (target)")
  ax.bar(
      x,
      uniform,
      width,
  ...
  _style(ax)
  tv_uniform = 0.5 * sum(
      abs(u - t) for u, t in zip(uniform, target, strict=True))
  tv_fitted = 0.5 * sum(abs(f - t) for f, t in zip(fitted, target, strict=True))
  _title(
      ax, "Restricting to real parent keys need not cost the marginal",
...
def main() -> None:
  ...
  _check_palette()
  print(f"measured orphan rate {_measured_orphan_rate():.1%}; "

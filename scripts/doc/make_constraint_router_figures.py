...
SPACE_C2E = 3 * 16**20  # ^(C2E[13][0-9A-F]{20}|7301[0-9A-F]{20})$
...
def fig_pk_blocker():
  ...
  rows = (
      ...
      ("C2E/7301 space (PK_COL)", SPACE_C2E, BLUE),
  ...
  for yi, (_, v, _) in zip(y, rows, strict=True):
    ax.text(
        v * 1.8,
        yi,
        f"{v:.2e}" if v >= SCI_NOTATION_MIN else f"{v:,}",
        va="center",
        color=INK,
        fontsize=8.5)
  ...
  handles = [
      plt.Rectangle((0, 0), 1, 1, color=c) for c in (ORANGE, AQUA, MUTED)
  ]
  ax.legend(
      handles, [
          f"GPU-resident ({GPU_SECONDS} GPU-s billed here)",
          "CPU work (llm_route_unused x"
          f"{LLM_ROUTE_UNUSED_WORKERS} workers)", "platform overhead"
      ],
      frameon=False,
      fontsize=8.5,
      labelcolor=INK,
  ...
  fig.savefig(
      ASSETS / "constraint-router-pk-blocker.png", dpi=160, facecolor=SURFACE)
  plt.close(fig)
...
def fig_outcomes():
  ...
  ax.set_xticks(x, [c for c, *_ in CONSTRAINT_COLS], fontsize=9, color=MUTED)
  ax.set_ylim(0, 265)
  ax.set_ylabel("T4 seconds per 512-value pool", color=MUTED, fontsize=9)
  handles = [
      plt.Rectangle((0, 0), 1, 1, color=route_color[k])
      for k in ("pattern-guided", "prose (charset)", "binary fallback")
  ]
  ax.legend(
      handles,
      ["pattern-guided (xgrammar)", "prose clause", "binary fallback (no LLM)"],
      frameon=False,
      fontsize=8.5,
      labelcolor=INK)
  ...
  ax.bar(x, landed, width=0.55, color=[route_color[k] for _, k, *_ in order])
  ax.axhline(LANDED, color=INK, linewidth=1, linestyle=":")
  ax.text(
      len(order) - 0.55,
      LANDED + 8,
      f"rows in sample {LANDED}",
      color=INK,
      fontsize=8,
      ha="right")
  ...
  ax.set_ylabel("distinct values landed (of 512 rows)", color=MUTED, fontsize=9)
  _style(ax, grid_axis="y")
  _title(
      ax, "The fallback pool is verbatim source",
      "COL_047 landed 58 distinct real values — its clause said "
      "'never copied'")

  fig.tight_layout(pad=1.6)
  fig.savefig(
      ASSETS / "constraint-router-outcomes.png", dpi=160, facecolor=SURFACE)
  plt.close(fig)

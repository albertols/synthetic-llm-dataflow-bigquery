...
def panel_cap(ax, cap, candidates, fanout, jx, jy, *, first):
    ...
    ax.plot(xs, frontier, color=INK, linewidth=1.6, zorder=4,
            label="wrap frontier  k = min(c, cap)")
    ax.plot(diag, diag, color=BLUE, linewidth=1.1, linestyle=":", zorder=4,
    ...
    for mask, color, label in (
        ...
        ax.scatter(candidates[mask] * jx[mask], fanout[mask] * jy[mask],
                   s=6, color=color, alpha=0.45, linewidths=0, zorder=3,
                   label=label)
    ...
    ax.text(cap * 1.35, cap * 0.42, f"cap = {cap}", color=MUTED, fontsize=8.8,
            zorder=5,
    ...
    ax.set_xlabel("candidates the parent holds for the shared value (c)",
                  color=MUTED, fontsize=9.5)
    if first:
        ax.set_ylabel("children per shared value (fan-out k)",
                      color=MUTED, fontsize=9.5)
    _style(ax)
    _title(
        ax,
...
def fig_candidate_cap():
    ...
    fig, axes = plt.subplots(1, len(CAPS), figsize=(16.2, 5.8),
    ...
    fig.legend(handles, labels, loc="lower center", ncol=5, frameon=False,
               fontsize=9.5, labelcolor=INK, bbox_to_anchor=(0.5, -0.015),
               markerscale=2.6)
    fig.suptitle(
        "The candidate cap only bites where the fan-out AND the parent "
        "both exceed it",
        color=INK, fontsize=14, fontweight="600", x=0.006, ha="left", y=0.995,
    )
    fig.tight_layout(w_pad=2.2, rect=(0, 0.055, 1, 0.965))
    fig.savefig(ASSETS / "multi-parent-candidate-cap.png", dpi=180,

...
def fig_verified():
  ...
  for xi, (a, b, c) in enumerate(zip(pre, now, src, strict=True)):
    for dx, v in ((-w, a), (0, b), (w, c)):
      ax.text(
          xi + dx, v * 1.15, f"{v:.3f}", ha="center", color=INK, fontsize=7.5)
  ax.set_yscale("log")
  ax.set_xticks(x, labels, fontsize=9, color=MUTED)
  ...
  ax = axes[2]
  x = np.arange(2)
  ax.bar(x, [BLOCKERS_BEFORE, BLOCKERS_AFTER], width=0.5, color=[ORANGE, AQUA])
  ...
  _style(ax)
  _title(ax, "False BLOCKERs gone", "numeric-domain exemption (A_TABLE)")
...
def fig_scrub_anatomy():
  ...
  ax.bar(
      x, redraw, width=w, color=ORANGE, label="resolved by REDRAW (v1 order)")
  ax.bar(
      x, nudged, width=w, bottom=redraw, color=BLUE, label="resolved by nudge")
  ...
  for xi, (_, coll, *_rest) in enumerate(SCRUB):
    ax.text(
        xi,
        coll + 12,
        f"{coll:.0f} collisions",
        ha="center",
        color=INK,
        fontsize=9)
  ax.set_xticks(x, labels, fontsize=9, color=MUTED)
  ax.set_ylabel(
      "per 1000-draw batch (run-4 telemetry)", color=MUTED, fontsize=9)
  _style(ax)
  ...
  ax.set_xticks(x, ["pre-scrub", "redraw-first scrub"], fontsize=9, color=MUTED)

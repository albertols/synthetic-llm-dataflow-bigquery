...
def fig_evidence():
  ...
  for i, c in enumerate(cols):
    ...
    ax1.text(
        ...
        color=INK,
        fontsize=9)
  ax1.set_yticks(list(y), [f"{c} ({RECALL_FAILERS[c][0]})" for c in cols])
  ax1.invert_yaxis()
  ax1.set_xlim(0, 1.0)
  ...
  ax2.barh([i + 0.19 for i in y2],
           src,
           height=0.34,
           color=BLUE,
           zorder=3,
           label="source distinct")
  ax2.barh([i - 0.19 for i in y2],
           syn,
           height=0.34,
           color=ORANGE,
           zorder=3,
           label="synthetic distinct")
  ax2.axvline(POOL_CAP, color=INK, linewidth=1.0, linestyle="--", zorder=4)
  ax2.text(
      POOL_CAP * 1.25,
      -0.42,
      f"LLM pool cap = {POOL_CAP}",
      color=INK,
      fontsize=9,
      va="center")
  ax2.set_xscale("log")
  ...
  fig.savefig(
      ASSETS / "prompt-constraints-evidence.png", dpi=160, facecolor=SURFACE)
  plt.close(fig)
...
def fig_mask_collapse():
  ...
  def mask(v: str) -> str:
    return "".join("9" if c.isdigit() else "A" if c.isupper() else c for c in v)
  ...
  table_hits = sum(
      mask(sample_mask_table(table, alphabets, rng.randrange)) in observed_masks
      for _ in range(DRAWS))
  ...
  _title(
      ...
      f"'C2E' prefix — {DRAWS} draws per sampler",
  )
  fig.tight_layout(pad=1.6)
  fig.savefig(
      ASSETS / "prompt-constraints-mask-collapse.png",
      dpi=160,
      facecolor=SURFACE)
  plt.close(fig)

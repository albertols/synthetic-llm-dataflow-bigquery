...
def fig_entropy_skew():
  ...
  for ax, p, label in (
    ...
    colors = [
        ORANGE if x == max(p) and label == "collapsed" else BLUE for x in p
    ]
    ax.bar(cats, p, color=colors, width=0.62)
    _style(ax)
    _title(
        ax, f"{label} enum", f"distinct=4 in BOTH · H={h:.2f} bits · "
  ...
  for p, color, label in ((P_BALANCED, BLUE, "balanced"), (P_COLLAPSED, ORANGE,
                                                           "collapsed")):
...
def fig_inverse_cdf():
  ...
  ax.hist(
      source, bins=bins, color=BLUE, density=True, alpha=0.35, label="source")
  ax.hist(
      inv_draws,
      bins=bins,
      histtype="step",
...
def fig_epoch_deciles():
  ...
  month_starts = np.cumsum(
      [0] + [calendar.monthrange(2026, m)[1] for m in range(1, 13)])
  ...
  _style(ax)
  _title(
      ax, "bursty timestamps, adaptive deciles",
...
def fig_null_patterns():
  ...
  marginals = [
      sum(f
          for p, f in NULL_PATTERNS_TRUE.items()
          if p[i] == "1")
      for i in range(3)
  ]
  ...
  independent = [
      math.prod(m if b == "1" else 1 - m
                for b, m in zip(p, marginals, strict=True))
      for p in patterns
  ]
  ...
  _style(ax)
  _title(
      ax, "null patterns: joint truth vs independence",
  ...
  ax.set_xlabel(
      "is-null bitstring over columns (A,B,C)", color=MUTED, fontsize=9)

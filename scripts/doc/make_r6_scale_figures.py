...
SOURCE_FILTER_DISTINCT = (3_030, 4_022, 45_708, 144_039, 73_231
                         )  # freetext_pool_source_filter size=
...
def fig_pool_race():
  ...
  rows.append(("thread 1: setup + fit-wait", RACE_T1_SETUP, RACE_T1_RAISE,
               ORANGE, "5 x 20 s waits → raise at t+191"))
  rows.append(("thread 2: setup → vllm_ready", RACE_T1_RAISE, RACE_VLLM_READY,
               BLUE, "fittable at t+251 (0.85, 14.1 GiB free)"))
  ...
  rows.append(("attempt 2: re-embed", RACE_REEMBED[0], RACE_REEMBED[1], ORANGE,
               "155 s (23 s cold)"))
  rows.append(("attempt 2: cache hits x3", RACE_CACHE_HITS[0],
               RACE_CACHE_HITS[-1], BLUE, ""))
  rows.append(("ladder A_COL_015 (attempt 2)", RACE_A15_LADDER[0],
               RACE_A15_LADDER[1], AQUA, "70 s"))
  n = len(rows)
  ...
  ax.annotate(
      "",
      xy=(RACE_T1_SPAWN + NEW_WINDOW_S, n + 0.15),
  ...
  ax.set_yticks([])
  ax.set_xlabel(
      "seconds since the A_TABLE pool branch started (19:24:16Z)",
      color=MUTED,
      fontsize=9)
...
def fig_pool_targets():
  ...
  ax.axhline(
      POOL_CAP, color=MUTED, linewidth=1.0, linestyle=(0, (4, 3)), zorder=2)
  ax.text(
      len(POOL_COLUMNS) - 0.55,
      POOL_CAP * 1.12,
      f"cap {POOL_CAP}",
      color=MUTED,
      fontsize=8.5,
  ...
  for i, (d, t) in enumerate(
      zip(SOURCE_FILTER_DISTINCT, POOL_TARGET, strict=True)):
...
def fig_format_gate():
  ...
  ax1.set_ylabel(
      "format_rejected (LLM values the gate refused)", color=MUTED, fontsize=9)
  ...
  for yi, s, t in zip(y, src, syn, strict=True):
    ax2.text(
        s - 1.2, yi, f"{s}", ha="right", va="center", color=INK, fontsize=8.5)
    ax2.text(
        t + 1.2, yi, f"{t}", ha="left", va="center", color=INK, fontsize=8.5)
  ax2.axvline(
      A19_SOURCE_LEN["max"], color=BLUE, linewidth=1.2, linestyle=(0, (4, 3)))
  ax2.text(
      A19_SOURCE_LEN["max"] + 0.6,
      3.42,
      "35-char wall\n(p95 == max)",
      color=BLUE,
      fontsize=8.5,
  ...
  _title(
      ax2, "A_COL_019: the prose gate had no ceiling",
...
def fig_where_time_went():
  ...
  ax.text(
      (GPU_BUSY_10M[1][0] + GPU_BUSY_10M[1][1]) / 2,
      n + 0.05,
  ...
  ax.legend(
      handles,
      ["Beam / IO", "GPU (embed + vLLM pool ladders)", "CPU generation"],
      frameon=False,
      fontsize=9,
      labelcolor=INK,
      loc="lower left",
...
def fig_length_ceiling_concept():
  ...
  wall = np.minimum(free,
                    CONCEPT_WIDTH)  # the same prose stored in a 35-char field
  ...
  fig, axes = plt.subplots(
      1, 3, figsize=(13.5, 4.4), facecolor=SURFACE, sharey=True)
  ...
  fig.suptitle(
      "length_ceiling fires only where p95 == max AND the spread below is wide (max(4, max/4))",
      color=INK,
      fontsize=12,
      fontweight="600",
      x=0.01,
  ...
  fig.tight_layout()
  fig.savefig(
      ASSETS / "r6-scale-length-ceiling-concept.png",
      dpi=160,
      facecolor=SURFACE)
  plt.close(fig)
...
if __name__ == "__main__":
  ...
  for name in ("r6-scale-pool-race", "r6-scale-pool-targets",
               "r6-scale-format-gate", "r6-scale-where-time-went",
               "r6-scale-length-ceiling-concept"):

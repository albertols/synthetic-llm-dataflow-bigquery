...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("ddl_json", help="Path to the source table's _ddl.json (extract_ddl.py output).")
    p.add_argument("-o", "--out", default="landing_ddl.json",
                   help="Output path for the BQ JSON schema array (default: landing_ddl.json).")
    p.add_argument("--print-bq", metavar="TABLE_FQN", default=None,
    ...
    if ts.partitioning:
        print(f"partitioning: {ts.partitioning.type} {ts.partitioning.field or ''}".rstrip())
    if ts.clustering:

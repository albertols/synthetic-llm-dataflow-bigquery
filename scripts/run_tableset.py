...
def emit_trigger_configs(
  ...
  waves_of_names = (
      ...
      if generate_fk_relationships else (_names(tables),))
  by_name = {t.rsplit(".", 1)[-1]: t for t in tables}
...
def run_waves(waves: list[list[list[str]]], max_parallel: int = 1) -> int:
...
def write_model_artifact(registry: RelationshipRegistry, table: str,
                         out_dir: Path) -> Path:
...
def plan_tableset(config: dict,
                  registry: RelationshipRegistry) -> list[list[str]]:
...
def main(argv: list[str] | None = None) -> int:
  ...
  artifact = write_model_artifact(registry, tables[0], Path(args.fk_models_dir))
  ...
  if args.emit_trigger_configs:
    for path in emit_trigger_configs(
        config,
        registry,
        Path(args.emit_trigger_configs),

...
matplotlib.use(
    "Agg")  # headless-safe: set before any pyplot import, module-wide.
...
def discover_artifact_sets(ref: str) -> dict[str, dict]:
  ...
  listing = _run_git("ls-tree", "-r", "--name-only", ref, "--", "docs/releases",
                     "integration_test")
  if not listing:
    return {}
  ...
  for line in listing.splitlines():
    ...
    elif (len(parts) == _EVIDENCE_PATH_PARTS and
          parts[:2] == ["docs", "releases"] and parts[3] == "evidence" and
          parts[5] == "real"):
      job_id = parts[4]
...
def _metric(base_value: Any, head_value: Any) -> dict[str, Any]:
  delta = (
      head_value -
      base_value if base_value is not None and head_value is not None else None)
...
def _crosscheck_diff_values(crosscheck: dict | None,
...
def compute_deltas(
  ...
  """Pure before/after metric diff between two artifact sets (each the
    ...
    """
  missing = [
      name for name, side in (("base", base), ("head", head)) if not side
  ]
  ...
  performance = {
      "execution_seconds":
          _metric(
              _execution_seconds(base_job_data),
              _execution_seconds(head_job_data)),
      "dominant_stage_seconds":
          _metric(
              _dominant_stage_seconds(base_job_data),
              _dominant_stage_seconds(head_job_data)),
      ...
      "counter_failed":
          _metric(
              _custom_counter(base_job_data, "failed"),
              _custom_counter(head_job_data, "failed")),
  }

  vllm = {
      "vllm_ignition_seconds":
          _metric(
              _vllm_ignition_seconds(base_job_data),
              _vllm_ignition_seconds(head_job_data)),
      "tokens_per_s":
          _metric(_tokens_per_s(base_job_data), _tokens_per_s(head_job_data)),
  }

  base_offline, head_offline = (base or {}).get("offline"), (head or
                                                             {}).get("offline")
  base_cross, head_cross = (base or
                            {}).get("crosscheck"), (head or
                                                    {}).get("crosscheck")
  base_stats, head_stats = (base or
                            {}).get("stats_diff"), (head or
                                                    {}).get("stats_diff")

  quality = {
      "dup_ratio_max":
          _metric(_dup_ratio_max(base_offline), _dup_ratio_max(head_offline)),
      "top_value_share_max":
          _metric(
              _top_value_share_max(base_offline),
              _top_value_share_max(head_offline)),
      "shape_recall_min":
          _metric(_shape_recall_min(base_cross), _shape_recall_min(head_cross)),
      "shape_precision_min":
          _metric(
              _shape_precision_min(base_cross),
              _shape_precision_min(head_cross)),
      "copy_fraction_max":
          _metric(
      ...
      "decile_ks_max":
          _metric(
              _stats_metric_max(base_stats, "decile_ks"),
              _stats_metric_max(head_stats, "decile_ks")),
  }
...
def _delta_tag(name: str, delta: float | None) -> str:
  ...
  if delta == 0:
    return "unchanged"
  improved = (_lower_is_better(name) and
              delta < 0) or (not _lower_is_better(name) and delta > 0)
...
# --------------------------------------------------------------------------
_CHANGE_TYPE_ORDER = ("feat", "fix", "perf", "refactor", "docs", "test",
                      "chore")
_CHANGE_TYPE_LABELS = {
    "feat": "Features",
    "fix": "Fixes",
...
def _render_change_summary(change_log: list[dict]) -> str:
  ...
  for entry in change_log:
    ...
    grouped.setdefault(change_type,
                       []).append(entry.get("title") or "(untitled commit)")
  ...
  for change_type in order:
    lines.append(
        f"### {_CHANGE_TYPE_LABELS.get(change_type, change_type.title())}")
...
_SECTION_HEADINGS = (("performance", "Performance"), ("vllm", "vLLM"),
                     ("quality", "Quality"))
...
def _render_before_after(deltas: dict) -> str:
  ...
  for side in missing:
    lines.append(
        f"- **{side.capitalize()}:** no integration run landed for this release."
    )
  if missing:
    lines.append("")
  lines.append(f"Base job: `{deltas.get('base_job') or 'n/a'}` · "
...
def _title_axes(ax, text: str, sub: str | None = None) -> None:
  ax.set_title(
      text, color=_INK, fontsize=12.5, fontweight="600", loc="left", pad=26)
  if sub:
    ax.text(
        0,
        1.025,
...
def _chart_step_time_before_after(deltas: dict, assets_dir: Path) -> Path:
  ...
  if not rows:
    ...
    width = 0.34
    bars_base = ax.bar(
        x - width / 2, base_vals, width, color=_BLUE, label="base", zorder=3)
    bars_head = ax.bar(
        x + width / 2, head_vals, width, color=_ORANGE, label="head", zorder=3)
...
def _evolution_points(
    deltas: dict, history: list[tuple[str, dict]]) -> list[tuple[str, float]]:
...
def _chart_metric_evolution(deltas: dict, history: list[tuple[str, dict]],
  ...
  if not points:
    ...
    ax.set_xticklabels(labels, rotation=15, ha="right", fontsize=9, color=_INK)
    ax.set_ylabel(
        f"{_HEADLINE_METRIC[1]} (head, seconds)", color=_MUTED, fontsize=9)
    ax.grid(axis="y", color=_GRID, linewidth=0.8, alpha=0.9)
    _style_axes(ax)
  _title_axes(ax, "Metric evolution across releases",
              f"{_HEADLINE_METRIC[1]} · lower is better")
  fig.tight_layout()
...
def make_charts(deltas: dict, history: list[tuple[str, dict]],
                assets_dir: Path) -> list[Path]:
...
def build_history(tags: list[str]) -> list[tuple[str, dict]]:
  ...
  for tag in tags:
    ...
    d = compute_deltas(
        prev_artifacts, cur_artifacts, base_job=prev_job, head_job=cur_job)
...
def _build_arg_parser() -> argparse.ArgumentParser:
  ...
  ap.add_argument(
      "--title",
      default=None,
      help="merge-commit title, for --print-next-version")
  ap.add_argument(
      "--prev", default=None, help="previous tag, for --print-next-version")
  ap.add_argument(
      "--base-ref",
      default=None,
      help="default: previous tag reachable from HEAD^")
  ap.add_argument("--head-ref", default="HEAD")
  ap.add_argument(
      "--version",
      default=None,
      help="default: computed from the head commit title")
  ap.add_argument(
      "--date",
      default=None,
...
def _resolve_base_ref(args: argparse.Namespace) -> str | None:
  ...
  described = _run_git("describe", "--tags", "--abbrev=0", "--match", "v[0-9]*",
                       f"{args.head_ref}^")
  return described.strip() if described and described.strip() else None


def _print_dry_run_plan(version: str, base_ref: str | None,
                        args: argparse.Namespace, report_path: Path,
...
def _write_redacted_report(report_path: Path, report_md: str,
                           head_artifacts: dict) -> redaction.Mapping:
  ...
  metrics_for_mapping = {
      "gcp": head_artifacts.get("gcp"),
      "offline": head_artifacts.get("offline")
  }
...
def _write_summary_and_index(version_dir: Path, out_dir: Path, version: str,
                             date: str, deltas: dict) -> None:
  (version_dir / "summary.json").write_text(
      json.dumps(
          {
              "version": version,
              "date": date,
              "headline_delta": _headline_delta(deltas)
          },
          indent=2,
      ) + "\n")
...
def main(argv: list[str] | None = None) -> int:
  ...
  head_title = (_run_git("log", "-1", "--format=%s", args.head_ref) or
                "").strip()
  ...
  deltas = compute_deltas(
      base_artifacts, head_artifacts, base_job=base_job, head_job=head_job)
  deltas["date"] = date
  ...
  if leaks:
    print(
        "release report leak scan failed — nothing committed:", file=sys.stderr)

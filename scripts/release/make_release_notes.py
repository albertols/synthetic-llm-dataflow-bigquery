...
def promote_unreleased(text: str, version: str, date: str) -> tuple[str, str]:
    ...
    body, tail = (rest[: next_h2.start()], rest[next_h2.start() :]) if next_h2 else (rest, "")
...
def fallback_notes(change_log: list[dict]) -> str:
    ...
    for entry in change_log:
        ...
        grouped.setdefault(category, []).append(_CONVENTIONAL_PREFIX_RE.sub("", title))

    if not grouped:
        return "_No changes recorded for this release._"

    sections = [
        "\n".join([f"### {category}", *(f"- {title}" for title in grouped[category])])
...
def absolutize_links(text: str, repo: str, ref: str) -> str:
    ...
    return _RELATIVE_LINK_RE.sub(
        lambda m: f"](https://github.com/{repo}/blob/{ref}/{m.group(1).lstrip('/')})", text
    )
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--version", required=True, help="tag being released, e.g. v0.3.1")
    ap.add_argument("--date", required=True, help="release date, YYYY-MM-DD")
    ap.add_argument("--prev", default=None, help="previous tag, for the compare link")
    ap.add_argument("--changelog", default="CHANGELOG.md", help="changelog to promote in place")
    ap.add_argument("--out", default=None, help="write the release body here (default: stdout)")
    ap.add_argument("--repo", default=_DEFAULT_REPO, help="owner/name, for absolute links")
    ap.add_argument("--head-ref", default=None, help="head sha/ref for the commit fallback")
    ap.add_argument(
        "--commit-title",
        action="append",
        default=None,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not section:
        ...
        if subjects is None:
            subjects = _git_subjects(args.prev, args.head_ref) if args.head_ref else []

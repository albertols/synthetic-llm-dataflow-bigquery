...
# --------------------------------------------------------------------------
def build_sample_sql(fqn: str, *, rows: int, run_id_col: str | None,
                     run_id: str | None) -> str:
  ...
  return (f"SELECT * FROM `{fqn}` t {where} "
...
def main(argv: list[str] | None = None) -> int:
  ...
  if not args.run_id_col and any(engine_labels.values()):
    _die("--engine-label carries a run_id but --run-id-col was not given "
  ...
  print(
      f"done: {written} total rows across {len(engine_labels)} sample file(s)")
  return 0

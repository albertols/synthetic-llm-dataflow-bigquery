...
_NETWORK_TAGS_RE = re.compile(
    r"use_network_tags(?:_for_flex_templates)?=([^'\"\]\s,]+)")
_FQN_PARTS = 3
...
class Mapping:
  ...
  def redact_text(self, text: str) -> str:
    ...
    for real, placeholder in self.values.items():
      ...
      if len(real) >= _MIN_TEXT_VALUE_LEN or (real.isupper() and
                                              real.isalpha()):
...
def build_mapping(
  ...
  pk_cols = set(offline.get("primary_key") or []) | set(
      bq.get("pk_columns") or [])
...
def _is_stats_diff_shaped(obj: Any) -> bool:
  """Structural check for a `stats_diff.json`-shaped dict
    ...
    registered."""
  return (isinstance(obj, dict) and isinstance(obj.get("columns"), dict) and
          "table" in obj)
...
def redact_csv(m: Mapping, text: str) -> str:
  ...
  for i, row in enumerate(rows):
    if i == 0:
      writer.writerow([m.columns.get(c, m.redact_scalar(c)) for c in row])
    else:
      writer.writerow([m.values.get(c, m.redact_scalar(c)) for c in row])
  return out.getvalue()

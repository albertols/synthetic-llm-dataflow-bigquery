...
def _column_report(df: pd.DataFrame, schema: list[dict[str,
                                                       str]]) -> dict[str, Any]:
  n = len(df)
  cols: dict[str, Any] = {}
  for field in schema:
    ...
    entry: dict[str, Any] = {
        ...
        "distinct":
            distinct,
        "distinct_ratio":
            round(distinct / n, 6) if n else 0.0,
        # SPARSITY
        "null_fraction":
            round(1.0 - (len(non_empty) / n), 6) if n else 0.0,
        "sentinel_fraction":
            round(sentinel_n / n, 6) if n else 0.0,
        "zero_fraction":
            round(zero_n / n, 6) if (n and is_numeric) else None,
        # REPETITION / SINGULARITY
        "top_value_share":
            round(top_count / n, 6) if n else 0.0,
        "is_constant":
            distinct <= 1,
        "is_near_constant":
            (top_count / n >= _NEAR_CONSTANT_SHARE) if n else False,
        "top_values": [{
            "value": str(idx)[:48],
            "count": int(cnt)
...
def _key_report(df: pd.DataFrame,
                pk_cols: list[str],
  ...
  key = (
      ...
      if len(present) > 1 else df[present[0]])
...
def _cross_overlap(frames: dict[str, pd.DataFrame],
...
def main(argv: list[str] | None = None) -> int:
  ...
  ap.add_argument(
      "--pk", default="", help="comma-separated primary-key columns")
  ap.add_argument(
      "--identity-cols",
      default="",

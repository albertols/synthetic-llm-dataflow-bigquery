...
class HistoryMappings:
  ...
  def _by_fqn(self, real_fqn: str) -> dict | None:
    return next((t for t in self.tables if t.get("real_fqn") == real_fqn), None)

  def _by_alias(self, alias: str) -> dict | None:
    return next((t for t in self.tables if t.get("alias") == alias), None)

  # -- mutation --------------------------------------------------------
  def assign_table(self, real_fqn: str, ddl_columns: list[str]) -> dict:

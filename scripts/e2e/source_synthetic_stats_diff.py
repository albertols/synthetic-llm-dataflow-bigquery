...
def build_sample_sql(fqn: str, *, rows: int) -> str:
  ...
  return (f"SELECT * FROM `{fqn}` t "
          "ORDER BY FARM_FINGERPRINT(TO_JSON_STRING(t)) "
          f"LIMIT {int(rows)}")


def fetch_rows_and_schema(client, fqn: str, *,
                          rows: int) -> tuple[list[dict], list]:
...
def build_table_schema(fqn: str, schema_fields: list) -> tuple[Any, list[str]]:
  ...
  for f in schema_fields:
    if f.field_type in _UNSUPPORTED_BQ_TYPES:
      ...
      continue
    columns.append(
        FieldSchema(name=f.name, bq_type=f.field_type, mode="NULLABLE"))
...
def _verdict(entropy_gap: float, ks: float | None, parity_gap: float) -> str:
  ks_val = ks or 0.0
  if (entropy_gap > _WARN_ENTROPY_GAP * _FAIL_MULTIPLIER or
      ks_val > _WARN_DECILE_KS * _FAIL_MULTIPLIER or
      parity_gap > _WARN_PARITY_DELTA * _FAIL_MULTIPLIER):
    return "fail"
  if (entropy_gap > _WARN_ENTROPY_GAP or ks_val > _WARN_DECILE_KS or
      parity_gap > _WARN_PARITY_DELTA):
    return "warn"
  return "ok"
...
def diff_profiles(src: dict[str, dict], syn: dict[str, dict]) -> dict:
  ...
  for col in common:
    ...
    ks = (
        ...
        if s.get("deciles") and y.get("deciles") else None)
    null_delta = round(_get(s, "null_fraction") - _get(y, "null_fraction"), 6)
    empty_delta = round(
        _get(s, "empty_fraction") - _get(y, "empty_fraction"), 6)
...
def _score(entry: dict) -> float:
  parity = max(
      abs(entry.get("null_delta") or 0.0), abs(entry.get("empty_delta") or 0.0))
  return max(
      entry.get("entropy_gap") or 0.0,
      entry.get("decile_ks") or 0.0, parity)
...
def _issues(entry: dict) -> list[str]:
  ...
  if parity > _WARN_PARITY_DELTA:
    out.append(
        f"null/empty parity delta {parity:.3f} -> {_CODE_AREA['parity']}")
  return out
...
def render_md(diff: dict) -> str:
  ...
  ranked = sorted(
      columns.items(),
      key=lambda kv:
      (_VERDICT_RANK.get(kv[1].get("verdict", "ok"), 0), _score(kv[1])),
      reverse=True,
  )
  ...
  if table.get("source_rows") is not None or table.get(
      "synthetic_rows") is not None:
    a(f"> Source sample: {table.get('source_rows')} rows. "
  ...
  if not flagged:
    ...
    for col, e in flagged:
      issues = _issues(e) or [
          "flagged, but below the itemized signal thresholds"
      ]
      for issue in issues:
...
def _string_length_band(rows: list[dict], col: str) -> dict:
  values = [
      str(v) for v in (r.get(col)
                       for r in rows) if v is not None and str(v).strip() != ""
  ]
...
def main(argv: list[str] | None = None) -> int:
  ...
  ap.add_argument(
      "--synthetic-fqn", required=True, help="project.dataset.table")
  ...
  print(
      f"[1/3] fetching {args.rows:,} rows -> source ({args.source_fqn})…",
      file=sys.stderr)
  src_rows, src_fields = fetch_rows_and_schema(
      client, args.source_fqn, rows=args.rows)
  print(
      f"[2/3] fetching {args.rows:,} rows -> synthetic ({args.synthetic_fqn})…",
      file=sys.stderr)
  syn_rows, syn_fields = fetch_rows_and_schema(
      client, args.synthetic_fqn, rows=args.rows)

  src_schema, src_type_skipped = build_table_schema(args.source_fqn, src_fields)
  syn_schema, syn_type_skipped = build_table_schema(args.synthetic_fqn,
                                                    syn_fields)
  ...
  diff["table"]["skipped"] = sorted(
      set(diff["table"]["skipped"]) | set(src_type_skipped)
      | set(syn_type_skipped))
  ...
  for col, entry in diff["columns"].items():
    ...
    drift = (
        abs(src_band["p50"] -
            syn_band["p50"]) if src_band and syn_band else None)

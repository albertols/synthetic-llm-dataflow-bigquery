...
def preflight_adc(project: str):
  try:
    ...
    _die(
        f"missing GCP client libs ({e}). Install: pip install google-cloud-bigquery"
    )
  try:
...
def _caller_identity(creds) -> str:
  return getattr(creds, "service_account_email", None) or getattr(
      creds, "_account", None) or "unknown"
...
def _row_count(client, fqn: str) -> int:
  return next(
      iter(client.query(f"SELECT COUNT(*) AS n FROM `{fqn}`").result())).n


def _aggregate(client, fqn: str, columns: list[str],
  ...
  for col in columns:
    ...
    out[col] = {
        "n":
            getattr(r, f"{col}__n"),
        "null_n":
            getattr(r, f"{col}__null_n"),
        "empty_n":
            getattr(r, f"{col}__empty_n"),
        "distinct_n":
            getattr(r, f"{col}__distinct_n"),
        "len_min":
            getattr(r, f"{col}__len_min"),
        "len_max":
            getattr(r, f"{col}__len_max"),
        "len_avg":
            getattr(r, f"{col}__len_avg"),
        "top_values": [{
            "value": t.get("value"),
            "count": t.get("count")
        } for t in top],
...
def _sample_sql(fqn: str,
                columns: list[str],
                nonempty_col: str | None = None) -> str:
...
def _profile_sample(values: list[str], top_k: int) -> dict[str, Any]:
  ...
  return {
      "sample_n": n,
      "shapes": {
          k: v for k, v in shapes.most_common(top_k)
      },
      "shapes_mass": {
          k: v / n for k, v in shapes.most_common(top_k)
      },
      "shapes_collapsed": {
          k: v for k, v in shapes_c.most_common(top_k)
      },
      "shapes_collapsed_mass": {
          k: v / n for k, v in shapes_c.most_common(top_k)
      },
      ...
      "charclass_fraction": {
          k: cc.get(k, 0) / n
          for k in ("digit", "upper", "lower", "space", "punct")
      },
...
def _missing_shape_lists(src_mass: dict, syn_counts: dict,
                         shape_recall: float) -> tuple[list, int]:
  ...
  below_floor = sum(1 for s, m in src_mass.items()
                    if m < _SHAPE_MASS_MIN and s not in syn_counts)
...
def _diff_column(col: str, src_agg, syn_agg, src_prof,
  ...
  missing, missing_below_floor = _missing_shape_lists(src_mass, syn_counts,
                                                      shape_recall)
  spurious = sorted(
      ((s, m)
       for s, m in syn_mass.items()
       if m >= _SHAPE_MASS_MIN and s not in src_counts),
      key=lambda kv: -kv[1],
  )[:10]
  ...
  cc_delta = {
      k: (syn_cc.get(k, 0) - src_cc.get(k, 0))
      for k in ("digit", "upper", "lower", "space", "punct")
  }
  ...
  len_ratio = abs(syn_len_avg -
                  src_len_avg) / src_len_avg if src_len_avg else None
  ...
  if shape_recall < _SHAPE_RECALL_WARN:
    add(
        "HIGH",
        f"shape recall {shape_recall:.2f}: synthetic misses source formats "
        f"(top missing: {', '.join(s for s, _ in missing[:3]) or 'n/a'})")
  if shape_precision < _SHAPE_PRECISION_WARN:
    add(
        "HIGH",
        f"shape precision {shape_precision:.2f}: synthetic invents formats "
  ...
  if shape_head_tv > _SHAPE_MASS_TV_WARN and head_shapes:
    ...
    add(
        "MEDIUM", f"shape mass divergence {shape_head_tv:.2f} (head total "
        f"variation): '{worst}' holds {src_mass.get(worst, 0.0):.2f} of "
        f"source mass vs {syn_mass.get(worst, 0.0):.2f} synthetic")
  if src_null is not None and syn_null is not None and abs(
      syn_null - src_null) > _FRACTION_DELTA_WARN:
    add(
        "MEDIUM", f"null fraction {syn_null:.2f} vs source {src_null:.2f} "
        f"(delta {syn_null - src_null:+.2f})")
  if src_empty is not None and syn_empty is not None and abs(
      syn_empty - src_empty) > _FRACTION_DELTA_WARN:
    add(
        "MEDIUM", f"empty fraction {syn_empty:.2f} vs source {src_empty:.2f} "
        f"(delta {syn_empty - src_empty:+.2f})")
  if len_ratio is not None and len_ratio > _LEN_RATIO_WARN:
    add(
        "MEDIUM", f"mean length {syn_len_avg:.1f} vs source {src_len_avg:.1f} "
        f"(rel delta {len_ratio:.2f})")
  for k, d in cc_delta.items():
    if abs(d) > _FRACTION_DELTA_WARN:
      add(
          "LOW", f"charclass '{k}' presence delta {d:+.2f} "
          f"(source {src_cc.get(k, 0):.2f} → synthetic {syn_cc.get(k, 0):.2f})")
  if copy_fraction is not None and copy_fraction > _COPY_FRACTION_WARN and (
      src_distinct_ratio or 0) > _HIGH_CARDINALITY_RATIO:
    add(
        "HIGH",
        f"literal copy fraction {copy_fraction:.2f} on a high-cardinality column "
        f"(source distinct ratio {src_distinct_ratio:.2f}) — possible memorization"
    )
  if (src_distinct_ratio or
      0) > _HIGH_CARDINALITY_RATIO and (syn_distinct_ratio or
                                        0) < 0.5 * (src_distinct_ratio or 0):
    add(
        "MEDIUM",
        f"distinct ratio collapsed {syn_distinct_ratio:.3f} vs source "
  ...
  return {
      "source": {
          "rows": src_agg["n"],
          "null_fraction": src_null,
          "empty_fraction": src_empty,
          "distinct": src_agg["distinct_n"],
          "distinct_ratio": src_distinct_ratio,
          "len_min": src_agg["len_min"],
          "len_avg": src_len_avg,
          "len_max": src_agg["len_max"],
          "len_p05": src_prof.get("len_p05"),
          "len_p50": src_prof.get("len_p50"),
      ...
      "synthetic": {
          "rows": syn_agg["n"],
          "null_fraction": syn_null,
          "empty_fraction": syn_empty,
          "distinct": syn_agg["distinct_n"],
          "distinct_ratio": syn_distinct_ratio,
          "len_min": syn_agg["len_min"],
          "len_avg": syn_len_avg,
          "len_max": syn_agg["len_max"],
          "len_p05": syn_prof.get("len_p05"),
          "len_p50": syn_prof.get("len_p50"),
      ...
      "diff": {
          "shape_recall":
              round(shape_recall, 4),
          "shape_precision":
              round(shape_precision, 4),
          "shape_mass_tv":
              round(shape_mass_tv, 4),
          "shape_head_tv":
              round(shape_head_tv, 4),
          "missing_shapes_below_floor":
              missing_below_floor,
          "missing_shapes": [{
              "shape": s,
              "source_mass": round(m, 4)
          } for s, m in missing],
          "spurious_shapes": [{
              "shape": s,
              "synthetic_mass": round(m, 4)
          } for s, m in spurious],
          "charclass_delta": {
              k: round(v, 4) for k, v in cc_delta.items()
          },
          "mean_length_rel_delta":
              round(len_ratio, 4) if len_ratio is not None else None,
          "null_fraction_delta":
              round(syn_null - src_null, 4) if
              (src_null is not None and syn_null is not None) else None,
          "empty_fraction_delta":
              round(syn_empty - src_empty, 4) if
              (src_empty is not None and syn_empty is not None) else None,
          "copy_fraction":
              round(copy_fraction, 4) if copy_fraction is not None else None,
          "copy_fraction_raw":
              round(copy_fraction_raw, 4)
              if copy_fraction_raw is not None else None,
      },
      "findings": findings,
  }
...
def render_markdown(
  ...
  for col, e in ranked:
    ...
    if d["missing_shapes"]:
      ...
      tail_note = (f" — plus {tail} rarer source shapes below the "
                   f"{_SHAPE_MASS_MIN * 100:.0f}% mass floor" if tail else "")
      a("**Missing shapes** (in source, absent from synthetic): " +
        ", ".join(f"`{m['shape']}` ({m['source_mass'] * 100:.1f}%)"
                  for m in d["missing_shapes"]) + tail_note)
      a("")
    if d["spurious_shapes"]:
      a("**Spurious shapes** (invented by synthetic, absent from source): " +
        ", ".join(f"`{m['shape']}` ({m['synthetic_mass'] * 100:.1f}%)"
                  for m in d["spurious_shapes"]))
      a("")
...
def run(args) -> dict[str, Any]:
  ...
  if missing_cols:
    print(
        f"WARNING: skipping columns absent from one/both tables: {missing_cols}",
        file=sys.stderr)
  if not columns:
  ...
  src_sample = _sample(client, args.source_fqn, columns, args.sample_size,
                       src_rows)
  syn_sample = _sample(client, args.synthetic_fqn, columns, args.sample_size,
                       syn_rows)
  ...
  for col in columns:
    ...
    if _needs_nonempty_topup(len(src_vals), src_agg[col]):
      ...
      print(
          f"[topup] {col}: source sampled {len(src_vals)} non-empty; "
          f"re-sampling {substantive} substantive rows…",
          file=sys.stderr)
      rows_t = _sample(
          client,
          args.source_fqn, [col],
          min(args.sample_size, 5000),
          substantive,
          nonempty_col=col)
    ...
    if _needs_nonempty_topup(len(syn_vals), syn_agg[col]):
      ...
      substantive = (a["n"] or 0) - (a["null_n"] or 0) - (a["empty_n"] or 0)
      print(
          f"[topup] {col}: synthetic sampled {len(syn_vals)} non-empty; "
          f"re-sampling {substantive} substantive rows…",
          file=sys.stderr)
      rows_t = _sample(
          client,
          args.synthetic_fqn, [col],
          min(args.sample_size, 5000),
          substantive,
          nonempty_col=col)
...
def main(argv=None) -> int:
  ...
  p.add_argument(
      "--columns", required=True, help="comma-separated free-text column names")
  p.add_argument(
      "--project", default=None, help="defaults to the source project")

...
| 2 | `assets/prefix-vs-kcenter-coverage.png` | uniform-prefix vs greedy k-center at a 3 % illustration budget; rare-mode counts from the actual selection algorithm (`retrieve_kcenter_k`) run on the plotted data — regenerated since 2026-09-21 by `scripts/doc/make_rag_geometry_figures.py` (seeded `CONCEPT` block), which also fixed the clipped title |
...
The article-facing companions of these figures — the 3-D sphere and its
GIF, the six seed pickers, the seed budget, the fidelity/originality axis and
the per-setup cost — are generated by the same script
(`uv run --no-sync python3 scripts/doc/make_rag_geometry_figures.py`) and
used by [article 4](../articles/04-b1-rag-deep-dive.md).

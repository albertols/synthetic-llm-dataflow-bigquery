...
deferred (ADR 0019, ADR 0030) because the pool branch finishes inside the
...
This pipeline adopted vLLM through that door (ADR 0011) and then walked
out of it (ADR 0014), for a reason of *shape*: `RunInference` is a
...
- **Concurrency is what makes it affordable.** At 28 requests the engine
  ...
  job submits concurrently (ADR 0028 D6) instead of one column at a time:
...
- **Cache hit ratio.** vLLM reports the cumulative fraction of prompt
  ...
  the ADR 0028 evidence found: the pool prompt used to interpolate the
...
| `exact_chained` | the pre-ADR-0034 chain of three | same envelopes, same counts, ~3× the shuffle bytes | A/B only |
...
`ws6-run-timeline.png` are the ADR 0034 / WS5 / WS6 design assets, each

...
| R4 | `b1_rag` or `b2_library` | `vllm` | `t4` | `200` | `16` | `0.5` | derived (no override) | Plumbing-only: confirms the real vLLM server, GCS model pull, and BigQuery write path all work on T4 when `SDFB_MODEL_URI` points at `qwen3_4b_instruct_2507` (never Gemma — §1). Not a fidelity run; keep `num_rows` small. |

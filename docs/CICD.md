...
```
on: workflow_dispatch (BUILD_TYPE: Development | Release, push_to_artifactory: bool, TICKET: optional)

jobs:
  unlock-env    # gates only on RELEASE_dbc_prd via reusable workflow
  build:
    - checkout
    - branch → ENV_NAME + GOOGLE_PROJECT_CORP + SERVICE_ACCOUNT_STRING
...
```
on: workflow_dispatch (BUILD_TYPE, TICKET)
...
```
on: workflow_dispatch (project_version, table_fqn, num_rows, TICKET)
...
| Org secret | `GOOGLE_PROJECT_CORP_{DEV,UAT,PRD}` | Per-env GCP project where GSM secrets live |

...
| Finding (both R1s) | Fix | Next-run readout |
|---|---|---|
| Most "shape-mass" findings (COL_054 75→54, COL_024 68.6→7.7 "inversion", COL_015 "49% spurious alpha") were the crosscheck's `RAND()<p LIMIT` storage-front sample — exact full-table shares matched synthetic within 2 pp; COL_015 "contamination" retracted (`REEMBOLSOS T` is a genuine dominant source value, heads re-emit it by design) | crosscheck samples hash-ordered (deterministic); new `shape_mass_tv` + `missing_shapes_below_floor` diff keys | crosscheck `shape_mass_tv` ≈ 0 on COL_024/COL_054-class; `missing_shapes` never empty at recall < 0.9 |

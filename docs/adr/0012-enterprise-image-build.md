...
- **JFrog Artifactory is the only reachable registry**, and it currently mirrors `com/example/cuda:12.2.2-cudnn-runtime-ubuntu22.04` (and a 12.6.1/24.04) plus `docker-remote/apache/beam_python3.11_sdk:2.71.0` (**not** 2.73.0).
...
- **Networking prerequisites for the Dataflow run** (private-IP workers):
  ...
  - **JFrog image pull** rides the corporate interconnect via the `artifactory` + `egress-net` network tags; Dataflow authenticates with `--image-repository-{username,password}-secret-id` (GSM). The worker SA needs `secretmanager.secretAccessor`.
...
- **Costs / open items** (must be confirmed before the first green build):
  - Exact JFrog pull strings for the CUDA + Beam images (the `com/example/cuda/...` form is ambiguous on where the tag splits).

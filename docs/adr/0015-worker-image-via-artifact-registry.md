...
The original decision below pushed the image to **Artifact Registry only**. Workflow 1 now pushes the *same* image to **both** JFrog `docker-local` (archival / registry-of-record, the immutable `:<branch>-<sha>` tag) **and** Artifact Registry, in a single `docker build` with two tags.
...
Added prerequisite: the CI JFrog releaser creds must keep write access to `docker-local` (write-once — only fresh `:<branch>-<sha>` tags, never `:latest`).

## Context

ADR 0003 made the corporate JFrog Artifactory (`docker-local`) the single image registry, and ADR 0009 made one image serve both the Flex Template **launcher** and the Dataflow **workers**. At §11 the launcher pulls the image fine, but every worker fails the SDK-harness pull:

...
| 11 | **Source stats store** (WS8, 2026-08-05 spec WS-B) — `source_table_stats` table contract | verifies (missing table = SKIP by design — stats still land as milestone + JSON artifact; `--source-stats-table ''` omits) | `bq mk` with `config/bq_schema/synthetic_rag/source_table_stats.schema.json`; drifted table = ACTION (write_rows load job would fail mid-launch) |

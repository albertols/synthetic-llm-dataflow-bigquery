...
uv run python scripts/hello_synthetic_mlx.py \
    --ddl_path output/SRC_dataset/ddl_metadata_SRC_dataset_B_TABLE.json \
    --reference_table SRC_dataset.B_TABLE \
    --reference_limit 5 \
    --num_rows 10 \
...
uv run python -m sdfb_beam.cli.run_pipeline \
    --runner DirectRunner \
    --client_type mlx \
    --ddl_uri output/SRC_dataset/ddl_metadata_SRC_dataset_B_TABLE.json \
    --reference_table SRC_dataset.B_TABLE \
...
- `--reference_table` must be the **same table the DDL describes** (`SRC_dataset.B_TABLE`), or the reference rows won't match the schema.

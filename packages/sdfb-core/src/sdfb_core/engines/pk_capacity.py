...
def fk_key_sample_cap(num_rows: int, other_capacity: int | None) -> int:
...
def max_rows_under_share(capacity: int | None, share: float) -> int | None:
    """Largest ``num_rows`` whose expected duplicate share stays at or
    under ``share`` for ``capacity`` slots; ``None`` when nothing limits
    (unbounded capacity, or a share of 1.0)."""
    if capacity is None or share >= 1.0:
        return None
    if share <= 0.0:
        return 0
    lo, hi = 0, max(1, capacity * FK_KEY_SAMPLE_MARGIN * 10)
    while expected_duplicate_share(hi, capacity) <= share:
        hi *= 2
    while lo < hi:
        mid = (lo + hi + 1) // 2
        if expected_duplicate_share(mid, capacity) <= share:
            lo = mid
        else:
            hi = mid - 1
    return lo

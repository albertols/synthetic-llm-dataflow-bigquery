...
def _column_target(
    values: set, observed: Counter, total: int
) -> dict[Any, float]:
...
class FkKeyPool:
    ...
    def draw(self, n: int, rng, use_numpy: bool = False) -> list[tuple]:
        ...
        return [
            null_tuple
            if self.null_fraction > 0.0 and rng.random() < self.null_fraction
            else self.keys[min(bisect_right(self.cum, rng.random()), last)]
            for _ in range(n)
        ]
...
def build_fk_key_pools(
    ...
    return [
        FkKeyPool.from_reference(
            cols=p["cols"], keys=p.get("keys") or (), reference_rows=reference_rows
        )
        for p in payloads
        if p.get("cols")
    ]

...
# ADR 0028 E2F families) that is ~1e-36.
...
def _build(ops: list[Any]) -> _Node:
    ...
    for op, arg in ops:
        ...
        if name == "LITERAL":
            ...
            lo, hi, sub = arg
            if hi is _MAXREPEAT or int(hi) == getattr(
                _MAXREPEAT, "real", 2**32
            ):
...
class PatternSampler:
    ...
    def sample(self, rng: random.Random) -> str:
        ...
        for _ in range(_FAMILY_MAX_TRIES):
            ...
            if v.startswith(prefix):
                return v
        raise ValueError(
            f"family prefix {prefix!r} unreachable under pattern "
    ...
    def sample_unique(
        ...
        if self.capacity < n:
            raise ValueError(
                f"capacity {self.capacity} < requested unique {n}"
            )
...
class ByteTemplateSampler:
    ...
    def __init__(self, prefix: str, length: int) -> None:
        if len(prefix) > length:
            raise ValueError(
                f"prefix {prefix!r} longer than pinned length {length}"
            )
        self.prefix = prefix
        self.length = length
...
def _sample_unique(
    ...
    if len(out) < n:
        raise RuntimeError(
            f"unique sampling stalled at {len(out)}/{n} "

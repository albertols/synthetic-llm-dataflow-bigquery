...
class TableProjection:
    ...
    def basis(self) -> str:
        ...
        basis = (
            f"driven by {self.parent} — {keys} distinct keys x {mean} "
            f"mean fan-out on {self.edge or '(driving edge)'}"
        )
        if (
            self.parent_rows is not None
            and self.parent_keys is not None
            and self.parent_rows != self.parent_keys
        ):
            basis += (
                f" (its parent lands {self.parent_rows:,} rows over "
...
def _matched_share(
    histogram: Mapping[str | int, int] | None, source_parent_tuples: int | None
) -> float | None:
    ...
    *,
    ...
    if not parent:
        return TableProjection(
            table=table, rows=max(0, int(launch_rows)),
    ...
    common = {
        "table": table, "parent": parent, "edge": edge,
        "parent_rows": parent_rows, "parent_keys": keys,
        "mean_fanout": mean, "zero_share": _zero_share(histogram),
    ...
    if mean is None:
        return TableProjection(
            rows=None,
            undecided=(
                f"the source fan-out histogram for its driving edge "
    ...
    if not keys:
        return TableProjection(
            rows=None,
            undecided=(
                f"the distinct keys its driving parent {parent} lands are "
                f"unknown here, so there is nothing to multiply the "
                f"measured mean fan-out by"
            ),
            **common,  # type: ignore[arg-type]
        )
    return TableProjection(rows=round(keys * mean), **common)  # type: ignore[arg-type]
...
def _chain(
    projection: TableProjection, by_name: Mapping[str, TableProjection]
) -> list[str]:
    ...
    for p in projections:
        if p.matched_share is not None and p.matched_share < matched_share_floor:
            out.append(
                ProjectionWarning(
                    ...
                    table=p.table,
                    measurement=(
                        f"the source parent covers only "
                    ...
                    consequence=(
                        f"{p.table_name} generates from the matched "
            ...
            if factor > factor_ceiling:
                ...
                out.append(
                    ProjectionWarning(
                        ...
                        consequence=(
                            f"this one table is {share:.1%} of the run's "
        ...
        if p.zero_share is not None and p.zero_share > zero_share_ceiling:
            out.append(
                ProjectionWarning(
                    ...
                    table=p.table,
                    measurement=(
                        f"{p.zero_share:.1%} of the source parent's key "
                    ...
                    consequence=(
                        f"the count is right — the mean already divides by "
        ...
        if p.adjusted:
            keys = (
                ...
                if p.rows is not None and p.repeat_share is not None
                else None
            )
                ...
                ProjectionWarning(
                    ...
                    measurement=(
                        ...
                        + (
                            ...
                            f"rows repeat a key value"
                            if p.repeat_share is not None
                            else ", with no key-repeat share measured over "
                            "the declared PK this launch"
                        )
                    ),
                    consequence=(
                        "its projection assumes that repeat distribution "
                        "reproduces: "
                        + (
                            f"{p.rows:,} rows over about {keys:,} DISTINCT "
                            f"key values"
                            if keys is not None
                            else "its rows carry repeated key values"
                        )
                        + (
                            f", which is what {', '.join(children)} "
                            f"{'is' if len(children) == 1 else 'are'} sized "
                            f"from, not the row count"
                            if children
                            else ""
                        )
...
def projection_banner(
    ...
    *,
    ...
    rows_w = max(
        [len(f"{p.rows:,}") for p in projections if p.rows is not None]
    ...
    for p in projections:
        ...
        lines.append(
            f" {p.table_name:<{name_w}} | {count:>{rows_w}} | {pct:>6} | "
            f"{_bar(share) if p.rows is not None else '':<{BAR_WIDTH}} | "
            f"{p.basis}"
        )
    tail = (
        f"across {len(projections)} table(s)"
        if dominant is None or not total
        else (
            f"across {len(projections)} table(s) — "
    ...
    lines.append(
        f" {'TOTAL':<{name_w}} | {total:>{rows_w},} | {'100.0%':>6} | "
        f"{'':<{BAR_WIDTH}} | {tail}"
    )
    if warnings:
        lines.append(
            f" WARNINGS | {len(warnings)} — a measured relationship makes "

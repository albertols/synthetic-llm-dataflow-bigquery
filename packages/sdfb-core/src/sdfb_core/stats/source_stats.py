...
def _add_value_mix(entry: dict, counter: Counter, total: int) -> None:
    ...
    if not total:
        return
    entropy = -sum(
        (c / total) * log2(c / total) for c in counter.values()
    )
...
def _add_numeric(entry: dict, substantive: list[object]) -> None:
    ...
    entry["zero_fraction"] = round(
        sum(1 for f in ordered if f == 0.0) / count, 6
    )
...
def _add_temporal(entry: dict, parsed: list[datetime]) -> None:
    ...
    entry["temporal_min"] = lo.date().isoformat() if day_granularity else lo.isoformat()
    entry["temporal_max"] = hi.date().isoformat() if day_granularity else hi.isoformat()
    dow: list[int] = [0] * 7
    ...
    entry["hour_mix"] = (
        [] if day_granularity else [round(c / total, 4) for c in hours]
    )
    now = datetime.now(UTC)
    entry["future_fraction"] = round(
        sum(1 for d in parsed if d > now) / total, 6
    )
...
def _null_pattern_entry(names: list[str], reference_rows: list[dict]) -> dict:
    ...
    n = len(reference_rows)
    patterns = Counter(
        "".join("1" if r.get(name) is None else "0" for name in names)
    ...
    return {
        ...
        "null_pattern_mix": [
            [bits, round(c / n, 6)]
            for bits, c in patterns.most_common(_NULL_PATTERN_TOP_K)
        ],
        "stats_tier": "sample",
        "profiler_version": PROFILER_VERSION,
    }

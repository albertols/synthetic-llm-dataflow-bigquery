...
_REFERENCE: dict[str, list[dict]] = {
    "dim_a": [{"A_KEY": f"a{i % 8}", "A_VAL": i * 997} for i in range(40)],
    "dim_b": [{"B_KEY": f"b{i % 8}", "B_VAL": i * 991} for i in range(40)],
    "fact": [{"A_KEY": f"a{i % 8}", "B_KEY": f"zz{i % 8}",
              "LINE_NO": "123"[i % 3], "AMOUNT": i * 7} for i in range(40)],
    "top": [{"T": f"t{i % 8}", "T_VAL": i * 983} for i in range(40)],
    "left": [{"T": f"t{i % 8}", "L": "lmn"[i % 3], "L_VAL": i} for i in range(40)],
    "right": [{"T": f"t{i % 8}", "R": "rst"[i % 3], "R_VAL": i * 2} for i in range(40)],
    "bottom": [{"T": f"t{i % 8}", "L": "lmn"[i % 3], "R": "rst"[i % 3],
                "B_VAL": i * 3} for i in range(40)],
}

_ROWS = {"dim_a": 40, "dim_b": 40, "fact": 48, "top": 12,
...
_MEASURED: dict[str, dict] = {
    "fact": {"driving_cols": ["A_KEY"], "histogram": {"1": 5, "3": 5},
             "cells": {"cols": ["LINE_NO"], "rows": [["1"], ["2"], ["3"]],
                       "counts": [1, 1, 1]},
             "exact_cells": True},
    "left": {"driving_cols": ["T"], "histogram": {"1": 1, "2": 1},
...
def _build_specs(tmp_path: Path, registry, order: list[str]):
    ...
    for name in order:
        ...
        specs.append(
            TableSpec(config=config, reference_rows=reference,
                      **_sinks(tmp_path, name), parent_edges=edges)
        )

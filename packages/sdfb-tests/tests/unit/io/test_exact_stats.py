...
def test_emit_exact_returns_pool_sizing_hints(monkeypatch):
    merged = {
        "NOTES": {"distinct": 73_231, "stats_tier": "exact",
                  "empty_fraction": 0.9, "generation_plan": ""},
        "__table__": {"distinct": 2, "stats_tier": "sample",
                      "empty_fraction": 0.0, "generation_plan": ""},
    }
    monkeypatch.setattr(
...
def test_emit_exact_degrades_loudly_not_fatally(monkeypatch):
    ...
    monkeypatch.setattr(
        "sdfb_beam.io.exact_stats.compute_exact_stats", _boom
    )
    out = _emit_source_stats(
        _args("exact"), _SCHEMA, _rows(), PreflightResult((), (), None)
    )

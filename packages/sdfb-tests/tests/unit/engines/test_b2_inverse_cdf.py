    ...
    return TableSchema.model_validate(
        {
            ...
            "schema": [{"name": "C", "type": type_, "mode": "NULLABLE"}],
        }
    ).columns[0]
...
def test_skewed_numeric_marginal_survives_sampling():
    ...
    rows += [{"C": 1000.0}] * 100
    prof = profile_column(_field("FLOAT64"), rows)
    draws = EmpiricalBackend._sample_one(
        prof, 2000, np.random.default_rng(7), 0.5
    )
...
def test_integer_rounding_still_applies():
    ...
    draws = EmpiricalBackend._sample_one(
        prof, 100, np.random.default_rng(3), 0.5
    )
...
def test_temporal_burst_density_survives_sampling():
    ...
    rows = [
        {"C": date(2026, 6, 1 + (i % 28))} for i in range(280)
    ] + [{"C": date(2026, 1, 2 + i)} for i in range(20)]
    prof = profile_column(_field("DATE"), rows)
    assert prof.kind is ColumnKind.TEMPORAL
    assert len(prof.quantiles) == 11
    draws = EmpiricalBackend._sample_one(
        prof, 1000, np.random.default_rng(11), 0.5
    )

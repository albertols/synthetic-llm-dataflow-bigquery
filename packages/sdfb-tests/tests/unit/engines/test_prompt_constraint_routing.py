...
class TestB1ConstraintFields:
    _DESC = (
        '{"llm_prompt_constraint": {"format": "hex id", "length": 8, '
        '"pattern": "^[0-9A-F]{8}$", "examples": ["AB12CD34"]}}'
    )

    def _profile(self):
        rows = [{"COL": f"prose value number {i} entirely different {i}"} for i in range(60)]
    ...
    def test_constraint_examples_never_enter_the_pool(self) -> None:
        ...
        # must be rejected.
        rows = [
            {"COL": f"{(i * 2654435761) % 16**8:08X}"} for i in range(60)
        ]

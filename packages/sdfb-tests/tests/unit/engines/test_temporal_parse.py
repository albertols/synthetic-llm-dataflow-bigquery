...
_CANONICAL = {
    ...
    "%Y-%m-%dT%H:%M:%S.%f": ["2026-08-06T21:05:58.123456", "2026-08-06T21:05:58.5"],
...
def test_profiler_hot_paths_are_lock_free(monkeypatch) -> None:
    ...
    assert temporal_string_to_float("2026-08-06", "%Y-%m-%d") == datetime.strptime(
        "2026-08-06", "%Y-%m-%d"
    ).replace(tzinfo=temporal_parse.UTC).timestamp()
    assert detect_temporal_format(["2026-08-06", "2025-01-01"]) == "%Y-%m-%d"
    assert detect_temporal_format(["2026-08-06T21:05:58Z", "2025-01-01T00:00:00Z"]) == (
        "%Y-%m-%dT%H:%M:%SZ"
    )

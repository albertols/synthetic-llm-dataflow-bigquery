...
def _numeric_profile(values: list[float], *, integral: bool = False) -> ColumnProfile:
...
def _draw(profile: ColumnProfile, n: int, *, use_numpy: bool, similarity: float = 0.5):
...
def test_skewed_marginal_survives_sampling(use_numpy: bool) -> None:
    ...
    drawn = [v for v in _draw(_numeric_profile(values), 2000, use_numpy=use_numpy)]
...
def test_prefix_band_survives_one_outlier(use_numpy: bool) -> None:
    ...
    drawn = _draw(_numeric_profile(values, integral=True), 2000, use_numpy=use_numpy)
...
def test_seeded_draws_are_reproducible(use_numpy: bool) -> None:
    ...
    assert _draw(profile, 64, use_numpy=use_numpy) == _draw(

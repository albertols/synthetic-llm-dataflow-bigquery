...
E2F_PATTERN = r"^(E2F[13][0-9A-F]{20}|2301[0-9A-F]{20})$"
UUID4_PATTERN = (
...
class TestPatternSampler:
    def test_e2f_draws_fullmatch_the_pattern(self):
        s = compile_pattern_sampler(E2F_PATTERN)
        assert s is not None
        rng = random.Random(7)
        rx = re.compile(E2F_PATTERN)
        for _ in range(200):
    ...
    def test_e2f_capacity_is_exact(self):
        s = compile_pattern_sampler(E2F_PATTERN)
        assert s is not None
        # E2F[13]{20 hex} = 2*16**20, 2301{20 hex} = 16**20.
    ...
    def test_family_shares_steer_prefix_mass(self):
        families = (("E2F3", 0.57), ("E2F1", 0.42), ("2301", 0.007))
        s = compile_pattern_sampler(E2F_PATTERN, families=families)
        assert s is not None
        rng = random.Random(3)
        n = 4000
        ...
        share_e2f3 = sum(d.startswith("E2F3") for d in draws) / n
        share_2301 = sum(d.startswith("2301") for d in draws) / n
        # Uniform-over-language would give 2301 a 1/3 share; the family
        # weights must pull it to ~0.7%.
        assert share_e2f3 == pytest.approx(0.57, abs=0.05)
        assert share_2301 == pytest.approx(0.007, abs=0.01)
...
class TestByteTemplateSampler:
    ...
    def test_tails_are_not_printable_ascii_only(self):
        ...
        assert any(
            any(not (0x20 <= ord(ch) <= 0x7E) for ch in t) for t in tails
        )

...
def test_detects_iso_datetimes_with_t_separator():
    values = [f"2024-06-01T{h:02d}:{m:02d}:33" for h in range(24) for m in (5, 25)]
    assert detect_temporal_format(values) == "%Y-%m-%dT%H:%M:%S"


def test_detects_space_separated_datetimes_with_fraction():
    values = [f"2024-06-01 10:{m:02d}:33.{u:06d}" for m in range(30) for u in (1, 999)]
...
def test_detects_uuid_shape_with_literal_hyphens():
    ...
    for _ in range(20):
        ...
        assert re.fullmatch(r"[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-a[0-9a-f]{3}-[0-9a-f]{12}", out)

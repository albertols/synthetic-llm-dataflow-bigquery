...
def _hex_upper_ids(n: int, length: int = 24, seed: int = 7) -> list[str]:
    ...
    return [
        "".join(rng.choice("0123456789ABCDEF") for _ in range(length))
        for _ in range(n)
    ]
...
def shaped_schema() -> TableSchema:
    return TableSchema.model_validate(
        {
            "table_info": {"table_id": "demo.contracts"},
            "schema": [
                {"name": "id", "type": "INT64", "mode": "REQUIRED"},
                {"name": "event_date", "type": "STRING", "mode": "REQUIRED"},
                {"name": "device_key", "type": "STRING", "mode": "REQUIRED"},
            ],
            "primary_keys": ["id"],
        }
    )
...
def test_b1_date_shaped_column_samples_novel_in_range_strings(shaped_ctx, shaped_rows):
    engine = B1RagEngine(embedder=HashingEmbedder(dim=64))
...
def test_b1_identifier_column_generates_per_row_distinct(shaped_ctx, shaped_rows):
    engine = B1RagEngine(embedder=HashingEmbedder(dim=64))
    engine.setup(_RecordingClient(), shaped_ctx)
    out = list(engine.generate_batch(200, GenerationConfig(seed=13)))
    assert out
...
def test_b1_identifier_generation_is_seed_reproducible(shaped_ctx):
    engine = B1RagEngine(embedder=HashingEmbedder(dim=64))
    engine.setup(_RecordingClient(), shaped_ctx)
    a = [r.device_key for r in engine.generate_batch(20, GenerationConfig(seed=5))]
    b = [r.device_key for r in engine.generate_batch(20, GenerationConfig(seed=5))]
    assert a == b
...
def test_b2_identifier_column_skips_llm_and_preserves_format(
    ...
    values = hook.sample(profiles["device_key"], 100, GenerationConfig(seed=3), rng)
    assert client.calls == []
    assert all(re.fullmatch(r"[0-9A-F]{24}", v) for v in values)
    assert len(set(values)) == len(values)
    assert not set(values) & {r["device_key"] for r in shaped_rows}

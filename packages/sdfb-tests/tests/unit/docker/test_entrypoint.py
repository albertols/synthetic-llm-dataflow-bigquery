...
def _run_entrypoint(tmp_path: Path, args: list[str]) -> tuple[str, list[str]]:
    ...
    patched = script.replace(_BOOT_PATH, str(boot)).replace(_LAUNCHER_PATH, str(launcher))
...
def test_entrypoint_is_valid_shell():
    ...
    proc = subprocess.run(
        ["sh", "-n", str(_ENTRYPOINT)], capture_output=True, text=True, check=False
    )
...
def test_worker_flags_dispatch_to_boot(tmp_path):
    ...
    role, forwarded = _run_entrypoint(
        tmp_path,
        ["--id=1", "--logging_endpoint=localhost:1", "--control_endpoint=localhost:2"],
    )
    assert role == "WORKER"
...
def test_launcher_flags_dispatch_to_launcher(tmp_path):
    ...
    role, forwarded = _run_entrypoint(
        tmp_path, ["--template-container-args", "--foo=bar"]
    )
    assert role == "LAUNCHER"

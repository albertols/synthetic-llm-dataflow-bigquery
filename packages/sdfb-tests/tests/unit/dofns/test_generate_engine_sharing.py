...
def test_different_runs_get_their_own_engines(counting_engine, customers_schema):
...
def test_engine_is_torn_down_only_by_the_last_holder(
    counting_engine, customers_schema
):
    d1, d2 = _dofn(_ctx(customers_schema)), _dofn(_ctx(customers_schema))
    d1.setup()
...
def test_concurrent_setups_build_the_engine_once(counting_engine, customers_schema):
...
def test_deferred_fk_side_build_is_shared_too(counting_engine, customers_schema):

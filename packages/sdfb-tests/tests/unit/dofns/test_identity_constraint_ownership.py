...
`pattern=^(E2F[13][0-9A-F]{20}|2301[0-9A-F]{20})$`, the Tier-P sampler
...
_PATTERN = r"^(E2F[13][0-9A-F]{20}|2301[0-9A-F]{20})$"
_CLAUSE = (
    '{"llm_prompt_constraint": {"route": "llm", "pattern": '
    '"^(E2F[13][0-9A-F]{20}|2301[0-9A-F]{20})$", "prefix": "E2F"}}'
)
...
def _schema(constrained: bool = True) -> TableSchema:
    return TableSchema.model_validate(
        {
            "table_info": {"table_id": "p.d.C_TABLE"},
            "schema": [
                {
                    "name": "B_COL_008",
                    "type": "STRING",
                ...
                {"name": "PLAIN_ID", "type": "STRING", "mode": "REQUIRED"},

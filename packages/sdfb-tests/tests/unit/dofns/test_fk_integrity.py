...
def _run(dofn, records, side=None):
    out = list(
        dofn.process(records[0], **({"fk_side": side} if side else {}))
    )
    for r in records[1:]:
        out.extend(
            dofn.process(r, **({"fk_side": side} if side else {}))
        )
    return out
...
class TestOrphanDetection:
    ...
    def test_side_input_pools_are_accepted(self):
        ...
        valid, invalid = _split(
            _run(dofn, [{"CC": "FR", "BR": 30}, {"CC": "FR", "BR": 10}], side=_EDGES)
        )
        assert len(valid) == 1
    ...
    def test_side_input_pools_survive_a_reentered_setup(self):
        ...
        valid, invalid = _split(
            _run(dofn, [{"CC": "ES", "BR": 30}], side=_EDGES)
        )

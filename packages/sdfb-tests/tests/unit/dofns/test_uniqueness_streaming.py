...
def test_exact_mode_is_unchanged_and_is_the_default():
    with TestPipeline() as p:
        ...
        assert_that(out["unique"] | beam.Map(lambda r: r["v"]),
                    equal_to(["x", "y"]), label="unique")
        assert_that(out["duplicates"] | beam.Map(lambda d: d["rule_id"]),
                    equal_to(["row.duplicate", "row.duplicate"]), label="dups")

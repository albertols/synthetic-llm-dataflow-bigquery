...
def _labels_for(mode: str) -> str:
    ...
    _ = (
        p
        | beam.Create([{"pk": "k", "id": "i", "v": "x"}])
        | EnforceUniqueness(identity_columns=["id"], pk_columns=["pk"], mode=mode)
    )
...
def test_pk_survivor_is_the_min_digest_row_and_losers_carry_their_own_payload():
    ...
    with TestPipeline() as p:
        ...
        assert_that(
            out["unique"], equal_to([survivor, rows[2]]), label="unique"
        )
        assert_that(
            out["duplicates"]
...
def test_identity_survivor_ignores_pk_losers_when_choosing():
    ...
    r1 = next(
        {"pk": "K", "id": "B", "v": f"loser-{i}"}
        for i in range(64)
        if row_digest({"pk": "K", "v": f"loser-{i}"}) > row_digest({"pk": "K", "v": "0"})
    )
    rows = [r0, r1, r2]
    with TestPipeline() as p:
        out = (
...
def test_packed_barrier_lands_rows_as_the_original_dicts():
    rows = [
        {"pk": "K1", "id": "I1", "v": "x", "n": None},
        {"pk": "K1", "id": "I1", "v": "x", "n": None},  # row dup
        {"pk": "K2", "id": "I2", "v": "y", "n": 3},
    ]
    with TestPipeline() as p:
        out = (

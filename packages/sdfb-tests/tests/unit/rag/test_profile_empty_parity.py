...
def _schema(cols: list[dict]) -> TableSchema:
    return TableSchema.model_validate(
        {"table_info": {"table_id": "p.d.t"}, "schema": cols}
    )
...
def test_mostly_empty_freetext_column():
    rows = [{"NOTES": ""} for _ in range(90)] + [
        {"NOTES": f"REF {i:04d} SETTLED PAYMENT ORDER"} for i in range(60)
    ]
...
def test_null_and_empty_are_separate_fractions():
    rows = (
        [{"NOTES": None} for _ in range(20)]
        + [{"NOTES": ""} for _ in range(30)]
        + [{"NOTES": f"LONG DESCRIPTION VALUE NUMBER {i:03d}"} for i in range(55)]
    )
    prof = profile_columns(_schema(_string_col()), rows)["NOTES"]
...
def test_categorical_keeps_empties_in_categories():
    rows = (
        ...
        + [{"CODE": "A"} for _ in range(25)]
        + [{"CODE": "B"} for _ in range(25)]
    )

...
E2F_PATTERN = "^(E2F[13][0-9A-F]{20}|2301[0-9A-F]{20})$"
_PK_DESC = (
    '{"llm_prompt_constraint": {"route": "llm", '
    '"pattern": "' + E2F_PATTERN.replace("\\", "\\\\") + '", '
...
def _e2f_value(i: int) -> str:
    return f"E2F3{i:020X}"
...
def _schema(cols: list[tuple[str, str]]) -> TableSchema:
    return TableSchema.model_validate(
        {
            "table_info": {"table_id": "demo.routed"},
            "schema": [
                {
                    "name": name,
                    "type": "STRING",
...
class TestTierPattern:
    def _rows(self):
        return [{"KEY": _e2f_value(i)} for i in range(80)]
    ...
    def test_pk_column_is_unique_across_batches(self, caplog) -> None:
        engine, _ = _setup(
            [("KEY", _PK_DESC)], self._rows(), caplog, pk_columns=["KEY"]
        )
        a = [r.model_dump()["KEY"] for r in engine.generate_batch(800, GenerationConfig(seed=3))]
        b = [r.model_dump()["KEY"] for r in engine.generate_batch(800, GenerationConfig(seed=4))]

...
def test_ladder_exits_after_two_fully_rejected_rounds(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="sdfb.milestone"):
        y = _pool_llm_yield(
            client, "p", {}, _id_profile(), ["CHG000001"], target=512,
        )
...
def test_partially_rejected_rounds_keep_the_ladder_running():
    ...
    y = _pool_llm_yield(
        client, "p", {}, _id_profile(), ["CHG000001"], target=64,
    )
            ...
            "schema": [
                {
                    ...
                    "mode": "REQUIRED",
                    "description": (
                        '{"llm_prompt_constraint": {"format": "change ref",'
...
def _fixed_width_prose_profile(width: int = 35) -> ColumnProfile:
    ...
    vals = tuple(
        [f"{long_text} {i}"[:width] for i in range(30)]  # cut at the width
...
def test_prose_candidates_are_clamped_to_a_fixed_width_source_ceiling(caplog):
    ...
    short_value = "NOMINA A UN BENEFICIARIO NUEVO"  # 30
    ...
    with caplog.at_level(logging.INFO, logger="sdfb.milestone"):
        y = _pool_llm_yield(
            _Client(), "p", {}, _fixed_width_prose_profile(35), [], target=8,
        )

...
def test_kanon_keep_set_comes_from_source_frequencies() -> None:
    ...
    store = _FrequentAwareStore(
        {"acct": _DOMAIN}, {"acct": frozenset({str(_ENUM_VALUE)})}
    )
...
def test_scrub_is_nudge_first_when_neighbors_are_free() -> None:
    ...
    out = engine._scrub_numeric_collisions(
        "acct", _RedrawForbiddenSampler(), None, list(values), 0.5, True
    )

...
def test_short_populations_return_everything_under_every_strategy():
    ...
    for strategy in POOL_SEED_STRATEGIES:
        assert sorted(select_seed_examples(vectors, texts, 8, strategy=strategy)) == [
            "x",
            "y",
        ]
...
def _freetext_ctx(strategy: str) -> GenerationContext:
    return GenerationContext(
        table_schema=TableSchema.model_validate(
            {
                "table_info": {"table_id": "demo.t"},
                "schema": [{"name": "notes", "type": "STRING", "mode": "REQUIRED"}],
            }
        ),
        reference_rows=[
            {"notes": f"reference prose value number {i}"} for i in range(60)
        ],

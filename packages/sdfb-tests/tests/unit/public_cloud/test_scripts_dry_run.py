...
def test_env_sh_derives_names():
  out = subprocess.run(
      [
          "bash", "-c",
          f'source "{GCP_DIR}/env.sh" && echo "$PROJECT_ID|$IMAGE_URI|$TEMPLATE_PATH|$MODEL_URI"'
      ],
      capture_output=True,
      text=True,
      env=FAKE_ENV,
...
def test_all_scripts_pass_bash_syntax_check():
  scripts = sorted(GCP_DIR.glob("*.sh")) + sorted(
      (GCP_DIR / "lib").glob("*.sh"))
  assert scripts, "no scripts found"
  for s in scripts:
    r = subprocess.run(["bash", "-n", str(s)],
                       capture_output=True,
                       text=True,
                       check=False)
...
def test_stage_models_cloudbuild_yaml_is_valid_and_complete():
  ...
  assert cfg["options"][
      "logging"] == "CLOUD_LOGGING_ONLY"  # custom build SA requirement
...
def test_build_image_cloudbuild_yaml_builds_mainline_dockerfile_unmodified():
  cfg = yaml.safe_load(
      (GCP_DIR / "cloudbuild" / "build_image.yaml").read_text())

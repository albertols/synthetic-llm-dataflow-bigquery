...
_spec = importlib.util.spec_from_file_location("deployment_prerequisites", _SCRIPT)
dp = importlib.util.module_from_spec(_spec)
sys.modules[_spec.name] = dp  # register before exec so @dataclass can resolve annotations
...
def _args(tmp_path, ddl: Path, **extra) -> object:
    argv = [
        "--project", "demo", "--source-table", "demo.raw.customers",
        "--no-extract", "--ddl-json", str(ddl),
...
def test_gcp_steps_skip_when_client_unavailable(tmp_path, ddl_file, offline):
    ctx = dp.Ctx(args=_args(tmp_path, ddl_file,
                            staging_bucket="b-staging", templates_bucket="b-templates",
                            model_uri="gs://b/synthetic/models/gemma4/e4b-it/v1/"))
    for step in (dp.step4_bq_tables, dp.step5_staging_bucket,
                 dp.step6_templates_bucket, dp.step7_bq_datasets, dp.step8_others):
        step(ctx)
    gcp = [r for r in ctx.results if r.step[0] in {"4", "5", "6", "7"} or r.step in {"8a", "8b"}]
...
def test_step8c_action_when_config_missing(tmp_path, ddl_file):
    ctx = dp.Ctx(args=_args(tmp_path, ddl_file))   # schemas-dir is an empty tmp dir
    dp.step8_others(ctx)
...
def test_main_writes_report_and_returns_ko(tmp_path, ddl_file, offline, capsys):
    argv = [
        "--project", "demo", "--source-table", "demo.raw.customers",
        "--no-extract", "--ddl-json", str(ddl_file),
        "--schemas-dir", str(tmp_path / "schemas"),
        "--models-dir", str(tmp_path / "models"),
        "--report-dir", str(tmp_path / "out"),
        "--model-uri", "gs://b/synthetic/models/gemma4/e4b-it/v1/",   # → missing local → ACTION
    ]
    rc = dp.main(argv)

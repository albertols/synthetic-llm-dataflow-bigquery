...
def test_order_puts_parents_first_chain_and_diamond():
  order = _registry(_CHAIN).generation_order(("orders", "customers", "lines"))
...
def test_emit_trigger_configs_ordered_files(tmp_path):
  paths = _mod.emit_trigger_configs(_config(), _registry(_TWO_PAIRS), tmp_path)
  names = [p.name for p in paths]
  assert names == ["00_a_t.json", "01_c_t.json", "02_b_t.json", "03_d_t.json"]
...
def test_run_waves_parallel_and_abort(tmp_path):
  import sys as _sys
  ok = [_sys.executable, "-c", f"open(r'{tmp_path}/ok', 'a').write('x')"]
  fail = [_sys.executable, "-c", "raise SystemExit(3)"]
  never = [_sys.executable, "-c", f"open(r'{tmp_path}/never', 'w').write('x')"]

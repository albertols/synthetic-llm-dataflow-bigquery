...
def _projections():
    ...
    c = project_table(
        "p.d.C_TABLE", launch_rows=_LAUNCH_ROWS, parent="B_TABLE",
        edge="(D_COL_001,D_COL_024,D_COL_025,C_COL_009)->B_TABLE",
        parent_rows=b.rows, parent_keys=b.rows,
        # mean 4.0145 -> 846,891
        histogram=_histogram(210_958, 846_891),
    )
    e = project_table(
        "p.d.E_TABLE", launch_rows=_LAUNCH_ROWS, parent="B_TABLE",
    ...
    a = project_table(
        "p.d.A_TABLE", launch_rows=_LAUNCH_ROWS, parent="C_TABLE",
        edge="(D_COL_024,D_COL_025,C_COL_009)->C_TABLE",
        parent_rows=c.rows, parent_keys=c.rows,
        # mean 36.5302 -> 30,937,138
        histogram=_histogram(846_891, 30_937_138),
    )
    f = project_table(
        "p.d.F_TABLE", launch_rows=_LAUNCH_ROWS, parent="E_TABLE",
        edge="(D_COL_001)->E_TABLE",
        parent_rows=e.rows, parent_keys=210_958,
...
def test_an_underivable_projection_says_so_instead_of_printing_a_number():
    ...
    orphaned = project_table(
        "p.d.X_TABLE", launch_rows=100, parent="P_TABLE",
        edge="(K)->P_TABLE", parent_rows=100, parent_keys=100,
...
def test_a_projection_with_an_underivable_table_totals_what_it_knows():
    ...
    unknown = project_table(
        "p.d.X_TABLE", launch_rows=99, parent="B_TABLE",
        parent_rows=99, parent_keys=99, histogram={},
    )
...
def test_the_mean_is_the_histogram_mean_zero_bucket_included():
    ...
    p = project_table(
        "p.d.F_TABLE", launch_rows=10, parent="E_TABLE",
        parent_rows=10, parent_keys=210_958,
        histogram=_histogram(465_079, 1_215_948, zero=118_055),
    )
...
def _warnings(projections, launch_rows=_LAUNCH_ROWS):
    return {w.code: w for w in projection_warnings(
        projections, launch_rows=launch_rows
    )}
...
def test_a_clean_launch_warns_about_nothing():
    ...
    child = project_table(
        "p.d.C_TABLE", launch_rows=1_000, parent="B_TABLE", edge="(K)->B_TABLE",
        parent_rows=1_000, parent_keys=1_000,
...
def test_a_child_inside_the_factor_ceiling_is_silent():
    ...
    projections = _projections()
    exploding = {
        w.table_name for w in projection_warnings(
            projections, launch_rows=_LAUNCH_ROWS
...
def _spec(name, rows, *, parent="", child_cols=("D_COL_001",),
          histogram=None, parents_measured=None, adjustment=None):
    ...
    schema = TableSchema.model_validate({
        "table_info": {"table_id": f"p.src.{name}"},
    ...
    if parent:
        ...
        edges = (FkEdgeSpec(
            child_cols=tuple(child_cols), ref_cols=tuple(child_cols),
            parent_landing=f"p.d.{parent}", parent_table=parent, mode="fanout",
        ),)
    ...
    return TableSpec(
        config=config, reference_rows=[],
        landing_sink=None, dlq_sink=None,  # type: ignore[arg-type]
        parent_edges=edges,
...
def _launch_specs():
    ...
    return [
        ...
        _spec("C_TABLE", 846_891, parent="B_TABLE",
              histogram=_histogram(210_958, 846_891)),
        _spec("E_TABLE", 423_999, parent="B_TABLE",
              histogram=_histogram(_E_KEY_VALUES, _E_CHILDREN),
              parents_measured=_E_PARENT_TUPLES,
              adjustment=ModelAdjustment(
                  table="p.d.E_TABLE", change="pk_dropped",
        ...
        _spec("A_TABLE", 30_937_138, parent="C_TABLE",
              histogram=_histogram(846_891, 30_937_138)),
        _spec("F_TABLE", 439_889, parent="E_TABLE",
              histogram=_histogram(465_079, 1_215_948, zero=118_055)),
    ]


def _rows_and_keys():
    rows = {"p.d.B_TABLE": 210_958, "p.d.C_TABLE": 846_891,
            "p.d.E_TABLE": 423_999, "p.d.A_TABLE": 30_937_138,
            "p.d.F_TABLE": 439_889}
...
def test_the_launcher_projects_the_whole_launch_from_its_specs(caplog):
    ...
    with caplog.at_level(logging.INFO, logger="sdfb.milestone"):
        projections = report_row_projection(
            _launch_specs(), launch_rows=_LAUNCH_ROWS,
            rows_by_landing=rows, keys_by_landing=keys,
        )
...
def test_a_clean_launch_logs_the_block_and_no_warning_milestone(caplog):
    ...
    with caplog.at_level(logging.INFO, logger="sdfb.milestone"):
        report_row_projection(
            [_spec("B_TABLE", 1_000)], launch_rows=1_000,
    ...
    plan = rp.LaunchPlan(
        ...
        runs=tuple(
            rp.TableRun(
                ...
                run_id="proj", fk_parent_landing="p.d",
            )
            for s in specs
        ),
    )
    args, _ = rp.parse_args([
        "--reference_table", "p.src.B_TABLE",
        "--landing_table", "p.d.B_TABLE",
        "--dlq_table", "p.dq.dlq", "--num_rows", str(_LAUNCH_ROWS),
        "--run_id", "proj", "--client_type", "fake",

...
def partitioned_ddl_dict(narrow_ddl_dict: dict) -> dict:
    ...
    return {
        **narrow_ddl_dict,
        "partitioning": {"type": "DAY", "field": "signup_at"},
        "clustering": {"fields": ["email", "customer_id"]},
    }
...
def test_writes_bare_array_not_fields_wrapper(tmp_path, narrow_ddl_dict):
    ...
    assert {f["name"] for f in schema} == {
        "customer_id", "email", "signup_at", "lifetime_value",
    }
...
def test_print_bq_folds_in_partitioning_and_clustering(tmp_path, capsys, partitioned_ddl_dict):
    src = _write_ddl(tmp_path, partitioned_ddl_dict)
    out = tmp_path / "landing_ddl.json"

    rc = derive_landing_schema.main(
        [str(src), "-o", str(out), "--print-bq", "myproj:synthetic_data.landing"]
    )
...
def test_print_bq_omits_flags_when_ddl_has_none(tmp_path, capsys, narrow_ddl_dict):

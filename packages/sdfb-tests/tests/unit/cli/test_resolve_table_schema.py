...
def _schema_with(
    description: str, *, table_description: str = "", name: str = "COL_001"
) -> TableSchema:
    return TableSchema(
...
def test_source_descriptions_are_stripped_when_target_unreachable(
    ...
    _route(
        monkeypatch,
        {
            ...
            TGT: RuntimeError("landing table not found"),
        },
    )
    with caplog.at_level(logging.INFO, logger="sdfb.milestone"):
        got = run_pipeline.resolve_table_schema("", SRC, TGT)
...
def test_live_failure_falls_back_to_the_pin_and_keeps_its_descriptions(
    ...
    monkeypatch.setattr(
        run_pipeline, "load_ddl", lambda uri: _schema_with(_CONSTRAINT)
    )
    with caplog.at_level(logging.INFO, logger="sdfb.milestone"):
...
def test_target_overlay_wins_over_the_pin_in_offline_source_mode(
    monkeypatch,
):
    ...
    monkeypatch.setattr(
        run_pipeline, "load_ddl", lambda uri: _schema_with(_OLD_CONSTRAINT)
    )
    got = run_pipeline.resolve_table_schema("gs://b/ddl.json", SRC, TGT)
    assert got.columns[0].description == _CONSTRAINT
...
def test_unusable_pin_is_a_warning_when_live_works(monkeypatch, caplog):
    _route(
        monkeypatch,
        {SRC: _schema_with(""), TGT: _schema_with(_CONSTRAINT)},
    )
    monkeypatch.setattr(
        run_pipeline,
        "load_ddl",

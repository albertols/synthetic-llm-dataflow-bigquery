...
def test_leak_scan_catches_stats_diff_skipped_column_survivor(tmp_path):
  ...
  (tmp_path / "stats_diff_metrics.json"
  ).write_text('{"table": {"skipped": ["source_only_ssn_col"]}}')
  hits = red.leak_scan(tmp_path, m)
...
def _gcp_with_network_tags():
  return {
      "project":
          "real-project",
      "bigquery": {
          "source_fqn": "real-project.src_ds.tbl",
          "landing_fqn": "real-project.land_ds.tbl"
      },
      "dataflow": [{
...
def test_redact_text_scrubs_network_tags_from_experiments():
  ...
  out = red.redact_text(
      m,
      "use_network_tags=corp-net-a;corp-egress-b use_network_tags_for_flex_templates=corp-net-a"
  )

...
_SCRIPT = Path(
    __file__).parents[5] / "scripts" / "e2e" / "source_synthetic_stats_diff.py"
_spec = importlib.util.spec_from_file_location("source_synthetic_stats_diff",
                                               _SCRIPT)
...
def _profile(rows, cols):
  ...
  schema = TableSchema(
      ...
      columns=[
          FieldSchema(name=c, bq_type="STRING", mode="NULLABLE") for c in cols
      ],
  )

...
_SCRIPT = Path(
    __file__).parents[5] / "scripts" / "release" / "make_release_report.py"
...
def test_discover_artifact_sets_groups_by_job_and_parses_json():
  tree = _make_tree({
      "integration_test/2026-01-01_00_00_00-1/e2e_gcp_metrics.json":
          json.dumps({
              "dataflow": [{
                  "job_id": "abc",
                  "timing": {
      ...
      "integration_test/2026-01-01_00_00_00-1/stats_diff.json":
          json.dumps({
              "columns": {},
              "table": {},
              "evaluation": None
...
def test_discover_artifact_sets_supports_real_bundle_layout():
  ...
  tree = _make_tree({
      f"integration_test/{job}/real/gcp_metrics.json":
          json.dumps({"dataflow": [{
              "job_id": "abc"
          }]}),
      f"integration_test/{job}/real/offline_metrics.json":
          json.dumps({"engines": {}}),
      f"integration_test/{job}/real/stats_diff_metrics.json":
          json.dumps({"columns": {}}),
      f"integration_test/{job}/real/freetext_crosscheck_metrics.json":
          (json.dumps({"meta": {}})),
      # Redacted twin — must never be picked up as an artifact.
      f"integration_test/{job}/oss/gcp_metrics.json":
          json.dumps({"dataflow": [{
              "job_id": "REDACTED"
          }]}),
  })
...
def test_discover_artifact_sets_supports_release_evidence_layout():
  ...
  tree = _make_tree({
      f"docs/releases/v0.1.0/evidence/{job}/real/gcp_metrics.json":
          json.dumps({
              "dataflow": [{
                  "job_id": "abc",
                  "timing": {
      ...
      f"docs/releases/v0.1.0/evidence/{job}/real/offline_metrics.json":
          (json.dumps({"engines": {}})),
      # Redacted twin — must never be picked up as an artifact.
      f"docs/releases/v0.1.0/evidence/{job}/oss/gcp_metrics.json":
          json.dumps({"dataflow": [{
              "job_id": "REDACTED"
          }]}),
      "docs/releases/v0.1.0/report.md":
          "# not an artifact",
      "docs/releases/v0.1.0/summary.json":
          json.dumps({"version": "v0.1.0"}),
  })
  sets = rel.discover_artifact_sets(tree)
  assert set(sets.keys()) == {job}
  assert sets[job]["gcp"] == {
...
def test_compute_deltas_quality_from_offline_and_crosscheck_and_stats_diff():
  ...
  assert d["quality"]["top_value_share_max"] == {
      "base": 0.3,
      "head": 0.25,
      "delta": pytest.approx(-0.05)
  }
  assert d["quality"]["shape_recall_min"] == {
      "base": 0.9,
      "head": 0.95,
      "delta": pytest.approx(0.05)
  }
  assert d["quality"]["shape_precision_min"]["base"] == 0.8
  assert d["quality"]["copy_fraction_max"] == {
      "base": 0.01,
      "head": 0.0,
...
def test_compute_deltas_counters_resolve_real_namespaced_keys():
  ...
  d = rel.compute_deltas(base, head)
  assert d["performance"]["counter_yielded"] == {
      "base": 900,
      "head": 950,
      "delta": 50
  }
  assert d["performance"]["counter_failed"] == {
      "base": 50,
      "head": 10,
      "delta": -40
  }
...
def test_compute_deltas_tokens_per_s_zero_duration_is_none_not_zerodiv():
  ...
  job = {
      ...
      "engine_milestones": {
          "durations_seconds": {
              "vllm_engine_init->vllm_ready": 0.0
          }
      },
  }
...
def test_render_report_has_sections_and_no_placeholders():
  md = rel.render_report(
      "v0.2.0", [{
          "type": "feat",
          "title": "feat: x"
      }],
      _fixture_deltas(),
      history=[("v0.1.0", _fixture_deltas())])
  for section in ("# Release v0.2.0", "Change summary", "Before / after",
                  "Charts"):
...
def test_make_charts_writes_pngs(tmp_path):
  paths = rel.make_charts(_fixture_deltas(), [("v0.1.0", _fixture_deltas())],
                          tmp_path)
...
def test_print_next_version_mode(capsys):
  rc = rel.main(
      ["--print-next-version", "--title", "feat: x", "--prev", "v0.1.0"])
  assert rc == 0
...
# --------------------------------------------------------------------------
def test_resolve_base_ref_passes_semver_match_filter_to_git_describe(
    monkeypatch):
...
def test_resolve_base_ref_explicit_base_ref_skips_git_describe(monkeypatch):
  ...
  def boom(*_a):
    raise AssertionError(
        "_run_git should not be called with an explicit --base-ref")
...
def _tree_with_gcp_metrics(job_id: str, execution_seconds: float) -> str:
  return _make_tree({
      f"integration_test/{job_id}/e2e_gcp_metrics.json":
          json.dumps({
              "dataflow": [{
                  "job_id": job_id,
                  "timing": {
...
def test_main_redaction_gate_exits_3_and_cleans_up(tmp_path, monkeypatch):
  ...
  def fake_leak_scan(oss_dir, mapping):
    seen_at_scan_time.extend(
        sorted(p.name for p in oss_dir.rglob("*") if p.is_file()))

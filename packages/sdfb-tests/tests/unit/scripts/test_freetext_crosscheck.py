...
_SCRIPT = Path(
    __file__).parents[5] / "scripts" / "e2e" / "freetext_crosscheck.py"
...
def test_needs_nonempty_topup_decision():
  ...
  assert _mod._needs_nonempty_topup(
      sampled_nonempty=3, agg={
          "n": 1000,
          "null_n": 100,
          "empty_n": 800
      })
  assert not _mod._needs_nonempty_topup(
      sampled_nonempty=500, agg={
          "n": 1000,
          "null_n": 100,
          "empty_n": 300
      })
  assert not _mod._needs_nonempty_topup(
      sampled_nonempty=0, agg={
          "n": 1000,
          "null_n": 200,
          "empty_n": 800
      })
...
def test_diff_flags_missing_shapes_and_empty_gap():
  ...
  entry = _mod._diff_column(
      "C",
      _agg(1000, empty_n=900, len_avg=0.7),
      _agg(1000, len_avg=7.0),
      src,
      syn,
  )
...
def test_diff_copy_fraction_detects_memorization():
  ...
  assert any(f["severity"] == "HIGH" and "memorization" in f["message"]
...
def test_diff_surfaces_long_tail_missing_shapes():
  ...
  assert not any("top missing: n/a" in f["message"] for f in entry["findings"])
...
def test_diff_scores_shape_mass_inversion():
  ...
  assert (_mod._column_score(bad_entry) > _mod._column_score(good_entry))

...
_SCRIPT = Path(
    __file__).parents[5] / "scripts" / "e2e" / "e2e_validation_analysis.py"
_spec = importlib.util.spec_from_file_location("e2e_validation_analysis",
                                               _SCRIPT)
...
def test_entropy_empty_column_is_zero(tmp_path):
  ...
  assert rc == 0
  m = json.loads(out.read_text())
  assert m["engines"]["eng"]["columns"]["empty_col"][

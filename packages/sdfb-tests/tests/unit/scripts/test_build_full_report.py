...
def _make_bundle(tmp_path: Path, variant: str = "real") -> Path:
  ...
  (d /
   "freetext_crosscheck_report.md").write_text("# Crosscheck\n\nshapes ok.\n")
  (d / "prompt_constraint_recommendations.md"
  ).write_text("# Recommendations\n\nadd a pattern.\n")
...
def test_full_report_unifies_docs_and_annexes(tmp_path):
  ...
  assert (text.index('id="doc-report-md"') <
...
def test_full_report_discovers_future_files_after_canonical(tmp_path):
  ...
  assert text.index('id="doc-prompt-constraint-recommendations-md"'
                   ) < text.index('id="doc-custom-notes-md"')

...
def test_sdfb_milestone_re_ignores_uppercase_or_invalid_names(probe_module):
  ...
  assert probe_module._SDFB_MILESTONE_RE.search(
      "some unrelated log line") is None
...
def test_worker_log_milestones_captures_sdfb_generic_and_legacy(probe_module):
  ...
  result = probe_module._worker_log_milestones(session, "proj", "job-1",
                                               probe_module._DEFAULT_MILESTONES,
                                               {})
...
def test_worker_log_milestones_attributes_pool_ladder_per_column(probe_module):
  ...
  assert ladder["CONCEPT"][
      "freetext_pool_shape_topup"] == "2026-07-06T00:01:00Z"
              ...
              "display_data": [
                  ...
                  },
                  {
                      "key":
                          "dataflow_kms_key",
                      "value":
                          "projects/kms-proj/locations/r/keyRings/kr/cryptoKeys/ck"
                  },
                  {
                      "key":
                          "subnetwork",
                  ...
                  },
                  {
                      ...
                      "value":
                          "europe-docker.pkg.dev/corp-proj/repo/sdfb-python:oss-abc1234"
                  },
                  {
                      "key": "num_rows",
                      "value": "1000000"
...
def test_parse_engine_labels_maps_job_id_to_label(probe_module):
  out = probe_module._parse_engine_labels(
      ["b1-rag=job-123", "b2-library=job-456"])
...
# --------------------------------------------------------------------------
def test_memorization_flags_flags_high_copy_ratio_high_cardinality(
    probe_module):
  columns = {
...
def test_substantive_sql_excludes_empty_and_frequent_source_values(
    probe_module):
  ...
  sql = probe_module._substantive_copy_sql("`p.d.landing`", "`p.d.source`",
                                           "`c`")
  flat = " ".join(sql.split())

...
_SCRIPT = Path(__file__).parents[5] / "scripts" / "release" / "make_release_notes.py"
...
def _filled(*, added: list[str] | None = None, removed: list[str] | None = None) -> str:
...
def test_promote_renames_unreleased_heading_to_tag_and_date():
    text, _ = notes.promote_unreleased(_filled(added=["Fan-out."]), "v0.3.1", "2026-09-20")

    assert "## [v0.3.1] — 2026-09-20" in text


def test_promote_returns_only_the_section_body_without_its_heading():
    _, body = notes.promote_unreleased(_filled(added=["Fan-out."]), "v0.3.1", "2026-09-20")
...
def test_fallback_groups_conventional_commit_types_into_emoji_categories():
    body = notes.fallback_notes(
        [
            {"type": "feat", "title": "feat(relational): parent-driven fan-out"},
            {"type": "fix", "title": "fix: dtype guard"},
        ]
    )
...
def test_fallback_strips_the_conventional_commit_prefix_from_each_bullet():
    body = notes.fallback_notes([{"type": "feat", "title": "feat(relational): fan-out"}])
...
def test_fallback_skips_the_actions_own_release_report_commits():
    body = notes.fallback_notes(
        [
            {"type": "docs", "title": "docs(releases): v0.3.0 report [release-report]"},
            {"type": "feat", "title": "feat: fan-out"},
        ]
    )
...
# CLI
# ---------------------------------------------------------------------------
def test_cli_rewrites_the_changelog_in_place_and_writes_the_notes_file(tmp_path):
...
def test_cli_falls_back_to_commit_titles_when_the_block_was_left_empty(tmp_path):
    changelog = tmp_path / "CHANGELOG.md"
    ...
    rc = notes.main(
        [
            ...
            "--out", str(out),
            "--repo", REPO,
            "--commit-title", "feat(relational): fan-out",
        ]
    )
...
def test_absolutize_pins_a_relative_repo_link_at_the_released_tag():
    out = notes.absolutize_links("See [ADR 0036](docs/adr/0036-fanout.md).", REPO, "v0.3.1")
...
def test_render_body_absolutizes_the_section_it_publishes():
    body = notes.render_body("- See [ADR 0036](docs/adr/0036-fanout.md).", "v0.3.1", "v0.3.0", REPO)

...
def _schema(table: str, cols: list[tuple[str, ...]]) -> TableSchema:
    ...
    return TableSchema.model_validate(
        {"table_info": {"table_id": f"p.src.{table}"},
         "schema": [{"name": c[0], "type": c[1],
                     "mode": c[2] if len(c) > 2 else "REQUIRED"} for c in cols]}
    )
...
def _root(tmp_path: Path, name: str, key: str, rows: int) -> TableSpec:
    ...
    cfg = _cfg(schema, ref, name, num_rows=rows,
               pk_columns=(key,), identity_columns=(key,))
...
def test_star_fact_two_dimensions(tmp_path):
    ...
    fact_ref = [{"A_ID": f"a{i % 8}", "B_ID": f"zz{i % 8}", "SEQ": "s123"[i % 3],
                 "AMOUNT": i * 7} for i in range(40)]
    fact_cfg = _cfg(fact_schema, fact_ref, "star_fact", num_rows=80,
                    pk_columns=("A_ID", "B_ID", "SEQ"), uniqueness_mode="streaming",
                    fanout={"driving_cols": ["A_ID"], "histogram": {"1": 5, "3": 5},
                            "cells": {"cols": ["SEQ"], "rows": [["s1"], ["s2"], ["s3"]],
                                      "counts": [1, 1, 1]},
                            "exact_cells": True, "conditional": [], "candidate_cap": 64})
    specs = [
        _root(tmp_path, "star_dim_a", "A_ID", 40),
        _root(tmp_path, "star_dim_b", "B_ID", 30),
        TableSpec(config=fact_cfg, reference_rows=fact_ref, **_sinks(tmp_path, "star_fact"),
                  parent_edges=(
                      FkEdgeSpec(child_cols=("A_ID",), ref_cols=("A_ID",),
                                 parent_landing="p.land.star_dim_a", parent_pk=("A_ID",),
                                 mode="fanout", keys_per_batch=10),
                      FkEdgeSpec(child_cols=("B_ID",), ref_cols=("B_ID",),
                                 parent_landing="p.land.star_dim_b", parent_pk=("B_ID",),
                                 mode="side_input"),
                  )),
    ]
    _run(specs)
...
def _diamond_specs(tmp_path: Path, tag: str, right_histogram: dict,
    ...
    left_schema = _schema(f"{tag}_left", [("T", "STRING"), ("L", "STRING"), ("L_VAL", "INT64")])
    left_ref = [{"T": f"t{i % 8}", "L": "lmn"[i % 3], "L_VAL": i} for i in range(40)]
    right_schema = _schema(f"{tag}_right", [("T", "STRING"), ("R", "STRING"), ("R_VAL", "INT64")])
    right_ref = [{"T": f"t{i % 8}", "R": "rst"[i % 3], "R_VAL": i * 2} for i in range(40)]
    bottom_schema = _schema(f"{tag}_bottom", [("T", "STRING"), ("L", "STRING"),
                                              ("R", "STRING", rest_mode), ("S", "STRING"),
                                              ("B_VAL", "INT64")])
    bottom_ref = [{"T": f"t{i % 8}", "L": "lmn"[i % 3], "R": "rst"[i % 3],
                   "S": "uv"[i % 2], "B_VAL": i * 3} for i in range(40)]
    branch = {"histogram": {"1": 1, "2": 1},
              "cells": {"cols": ["L"], "rows": [["l1"], ["l2"]], "counts": [1, 1]},
              "exact_cells": True, "conditional": [], "candidate_cap": 64}
    left_cfg = _cfg(left_schema, left_ref, f"{tag}_left", num_rows=24,
                    pk_columns=("T", "L"), uniqueness_mode="streaming",
                    fanout={"driving_cols": ["T"], **branch})
    right_cfg = _cfg(right_schema, right_ref, f"{tag}_right", num_rows=24,
    ...
    return [
        ...
        TableSpec(config=left_cfg, reference_rows=left_ref,
                  **_sinks(tmp_path, f"{tag}_left"), parent_edges=(edge_top,)),
        TableSpec(config=right_cfg, reference_rows=right_ref,
                  **_sinks(tmp_path, f"{tag}_right"), parent_edges=(edge_top,)),
        TableSpec(config=bottom_cfg, reference_rows=bottom_ref,
...
def test_existence_filter_drops_unmatched_keys(tmp_path):
    ...
    child_cfg = _cfg(child_schema, child_ref, "ex_child", num_rows=32,
    ...
    specs = [
        TableSpec(config=_cfg(p_schema, p_ref, "ex_p", num_rows=60, seed=11,
                              pk_columns=("K",)),
                  reference_rows=p_ref, **_sinks(tmp_path, "ex_p")),
        TableSpec(config=_cfg(q_schema, q_ref, "ex_q", num_rows=60, seed=13,
                              pk_columns=("K",)),
                  reference_rows=q_ref, **_sinks(tmp_path, "ex_q")),
...
def _graph_specs(tmp_path: Path) -> list[TableSpec]:
    ...
    mid_schema = _schema("g_mid", [("RT", "STRING"), ("MD", "STRING"), ("M_VAL", "INT64")])
    mid_ref = [{"RT": f"r{i % 8}", "MD": "mno"[i % 3], "M_VAL": i} for i in range(40)]
    leaf_schema = _schema("g_leaf", [("RT", "STRING"), ("MD", "STRING"),
                                     ("LF", "STRING"), ("L_VAL", "INT64")])
    leaf_ref = [{"RT": f"r{i % 8}", "MD": "mno"[i % 3], "LF": "fg"[i % 2],
                 "L_VAL": i} for i in range(40)]
    fact_schema = _schema("g_fact", [("RT", "STRING"), ("MD", "STRING"), ("DM", "STRING"),
                                     ("FSEQ", "STRING"), ("F_VAL", "INT64")])
    fact_ref = [{"RT": f"r{i % 8}", "MD": "mno"[i % 3], "DM": f"zz{i % 8}",
                 "FSEQ": "qxy"[i % 3], "F_VAL": i} for i in range(40)]
    twin_schema = _schema("g_twin", [("RT", "STRING"), ("MD", "STRING"),
                                     ("LF", "STRING"), ("TW_VAL", "INT64")])
    twin_ref = [{"RT": f"r{i % 8}", "MD": "mno"[i % 3], "LF": "fg"[i % 2],
                 "TW_VAL": i * 5} for i in range(40)]
    mid_cfg = _cfg(mid_schema, mid_ref, "g_mid", num_rows=16, pk_columns=("RT", "MD"),
                   uniqueness_mode="streaming",
                   fanout={"driving_cols": ["RT"], "histogram": {"2": 1},
                           "cells": {"cols": ["MD"], "rows": [["m1"], ["m2"]],
                                     "counts": [1, 1]},
                           "exact_cells": True, "conditional": [], "candidate_cap": 64})
    leaf_cfg = _cfg(leaf_schema, leaf_ref, "g_leaf", num_rows=24,
                    pk_columns=("RT", "MD", "LF"), uniqueness_mode="streaming",
                    fanout={"driving_cols": ["RT", "MD"], "histogram": {"1": 1, "2": 1},
                            "cells": {"cols": ["LF"], "rows": [["f1"], ["f2"]],
                                      "counts": [1, 1]},
                            "exact_cells": True, "conditional": [], "candidate_cap": 64})
    fact_cfg = _cfg(fact_schema, fact_ref, "g_fact", num_rows=32,
                    pk_columns=("RT", "MD", "FSEQ"), uniqueness_mode="streaming",
                    fanout={"driving_cols": ["RT", "MD"], "histogram": {"1": 1, "3": 1},
                            "cells": {"cols": ["FSEQ"], "rows": [["q1"], ["q2"], ["q3"]],
                                      "counts": [1, 1, 1]},
                            "exact_cells": True, "conditional": [], "candidate_cap": 64})
    twin_cfg = _cfg(twin_schema, twin_ref, "g_twin", num_rows=24,
                    pk_columns=("RT", "MD", "LF"), uniqueness_mode="streaming",
                    fanout={"driving_cols": ["RT", "MD", "LF"], "histogram": {"1": 1},
                            "cells": None, "exact_cells": False,
                            "conditional": [], "candidate_cap": 64})
    return [
        _root(tmp_path, "g_root", "RT", 8),
        TableSpec(config=mid_cfg, reference_rows=mid_ref, **_sinks(tmp_path, "g_mid"),
                  parent_edges=(FkEdgeSpec(child_cols=("RT",), ref_cols=("RT",),
                                           parent_landing="p.land.g_root",
                                           parent_pk=("RT",), mode="fanout",
                                           keys_per_batch=10),)),
        TableSpec(config=leaf_cfg, reference_rows=leaf_ref, **_sinks(tmp_path, "g_leaf"),
                  parent_edges=(FkEdgeSpec(child_cols=("RT", "MD"), ref_cols=("RT", "MD"),
                                           parent_landing="p.land.g_mid",
                                           parent_pk=("RT", "MD"), mode="fanout",
                                           keys_per_batch=10),
                                # Carried through the driving edge (ADR 0036 D4):
                                # no DAG edge, not even a parent lookup.
                                FkEdgeSpec(child_cols=("RT",), ref_cols=("RT",),
                                           parent_landing="p.land.g_root",
                                           parent_pk=("RT",), mode="implied"))),
        _root(tmp_path, "g_dim", "DM", 10),
        TableSpec(config=fact_cfg, reference_rows=fact_ref, **_sinks(tmp_path, "g_fact"),
                  parent_edges=(FkEdgeSpec(child_cols=("RT", "MD"), ref_cols=("RT", "MD"),
                                           parent_landing="p.land.g_mid",
                                           parent_pk=("RT", "MD"), mode="fanout",
                                           keys_per_batch=10),
                                FkEdgeSpec(child_cols=("DM",), ref_cols=("DM",),
                                           parent_landing="p.land.g_dim",
                                           parent_pk=("DM",), mode="side_input"))),
        TableSpec(config=twin_cfg, reference_rows=twin_ref, **_sinks(tmp_path, "g_twin"),
                  parent_edges=(FkEdgeSpec(child_cols=("RT", "MD", "LF"),
                                           ref_cols=("RT", "MD", "LF"),
                                           parent_landing="p.land.g_leaf",
                                           parent_pk=("RT", "MD", "LF"), mode="fanout",
                                           keys_per_batch=10),)),
    ]
...
def test_graph_six_tables(tmp_path):
    ...
    assert _tuples(twin, "RT", "MD", "LF") <= _tuples(leaf, "RT", "MD", "LF")  # 1:1
    for rows, pk in ((root, ("RT",)), (mid, ("RT", "MD")), (leaf, ("RT", "MD", "LF")),
                     (dim, ("DM",)), (fact, ("RT", "MD", "FSEQ")),
                     (twin, ("RT", "MD", "LF"))):

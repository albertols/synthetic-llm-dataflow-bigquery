...
description: Subagent that interprets landed E2E runs — reads the two metrics JSONs in runs/<JOB_ID>/real/ (legacy runs: parent-level e2e_*_metrics.json) against the 8 RUN_PLAYBOOK pass criteria and writes verdict + postmortem + remediation handoff. No cloud access; extreme reasoning. Invoke after gcp-deploy-runner lands a run.
model: fable
